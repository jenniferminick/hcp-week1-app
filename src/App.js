...
const SAMPLE_ANSWERS = {
  ...
  knownFor: "slow drain detective",
  topServices: "drain cleaning and water heater installs",
...
function Card({ children, style }) {
  return (
    <div style={{ background: WHITE, borderRadius: 16, padding: 28, boxShadow: "0 4px 24px rgba(0,41,66,0.08)", marginBottom: 20, ...(style || {}) }}>
      {children}
    </div>
  );
}
...
function Btn({ children, onClick, variant, style, disabled }) {
  ...
  const vars = {
    primary: { background: YELLOW, color: NAVY },
    secondary: { background: NAVY, color: WHITE },
    success: { background: GREEN, color: WHITE },
    ghost: { background: GRAY100, color: GRAY600 },
  };
  ...
  return (
    <div>
      ...
      <Card>
        ...
        <p style={{ fontSize: 14, color: GRAY600, margin: "0 0 8px", lineHeight: 1.7 }}>{hint}</p>
        <p style={{ fontSize: 13, color: GRAY400, margin: "0 0 16px", fontStyle: "italic" }}>{q.placeholder}</p>
        <div style={{ position: "relative" }}>
          <textarea
            value={value}
...
function LeadEngagement({ onBack, onAmplify }) {
  ...
  function logLead(typeId, n) {
    const num = n || 1;
    const entries = [];
    for (let i = 0; i < num; i++) entries.push({ type: typeId, timestamp: Date.now() });
    setLog(p => [...p, ...entries]);
  }
    ...
    <div style={{ position: "fixed", inset: 0, background: GRAY100, zIndex: 300, overflowY: "auto" }}>
      <div style={{ background: NAVY, padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10, boxShadow: "0 2px 12px rgba(0,0,0,0.2)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ color: WHITE, fontWeight: 800, fontSize: 16, lineHeight: 1 }}>Coach Dashboard</div>
            <div style={{ color: YELLOW, fontSize: 12, fontWeight: 600 }}>{submissions.length} submissions</div>
          </div>
        </div>
...
export default function App() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (saved && saved.answers && Object.keys(saved.answers).length > 0) {
          ...
          if (typeof saved.postCount === "number") setPostCount(saved.postCount);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      try {
        localStorage.setItem("draft:answers", JSON.stringify({ answers, post, postCount }));
        setAutoSaved(true);
  ...
  useEffect(() => {
    if (appPhase === "groups" && groups5.length === 0 && !groupsLoading) {
      ...
      findFacebookGroups(city, 5)
        .then(r => { setGroups5(r); setGroupsLoading(false); if (!r.length) setGroupsError("Could not find groups. Try searching Facebook manually."); })
        .catch(() => { setGroupsLoading(false); setGroupsError("Search failed."); });
    }
      ...
      <div style={{ background: NAVY, padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 12px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {appPhase !== "lane" && (
            <>
              {autoSaved && (
                <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}>
          ...
          <div>
            <div style={{ background: NAVY, borderRadius: 16, padding: 32, marginBottom: 20, textAlign: "center" }}>
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                {[ht.stat1, ht.stat2, ht.stat3].map((s, i) => (
                  <div key={i} style={{ background: "rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 18px", fontSize: 13, color: WHITE }}>{s}</div>
                ))}
              </div>
            </div>
              ...
              {[
                {
                  ...
                  status: answeredN === 0 ? { label: ht.statusNotStarted, bg: GRAY100, fg: GRAY400 } : answeredN < 15 ? { label: ht.statusInProgress, bg: "#FEF9EC", fg: "#92400E" } : { label: ht.statusDone, bg: "#D1FAE5", fg: "#065F46" },
                },
                {
            ...
            <Card>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
                <button onClick={() => setAppPhase("ch1")} style={{ background: WHITE, border: "2px solid " + NAVY, borderRadius: 14, padding: 20, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 16 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 800, color: NAVY, fontSize: 15, marginBottom: 4 }}>{ht.typeTitle}</div>
                    <div style={{ fontSize: 13, color: GRAY600, lineHeight: 1.5 }}>{ht.typeDesc}</div>
                  </div>
                </button>
                <button onClick={() => setAppPhase("voice")} style={{ background: NAVY, border: "2px solid " + NAVY, borderRadius: 14, padding: 20, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 16 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 800, color: YELLOW, fontSize: 15, marginBottom: 4 }}>{ht.voiceTitle}</div>
                    <div style={{ fontSize: 13, color: GRAY400, lineHeight: 1.5 }}>{ht.voiceDesc}</div>
                  </div>
                </button>
              </div>
            </Card>
        ...
        {appPhase === "replicate" && (
          <>
            ...
            <Card>
              ...
              {postCount > 0 && postCount < 9 && (
                <div style={{ background: "#FEF9EC", border: "1.5px solid " + YELLOW, borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: GRAY800, lineHeight: 1.6 }}>
                  You have posted in <strong>{postCount + 1} total groups</strong>. You still have <strong>{9 - postCount} more to go</strong>. Keep going.
                </div>
              )}
              {postCount === 9 && !tenDone && (
