...
async function findFacebookGroups(city, count) {
  ...
  try {
    ...
    return Array.isArray(parsed) ? parsed.filter(g => g.name).map(g => Object.assign({}, g, { url: "https://www.facebook.com/search/groups/?q=" + encodeURIComponent(g.name) })) : [];
  } catch(e) { return []; }
}
...
// ── Coach Dashboard (Vercel KV–aware stub) ────────────────────────────────────
    ...
    <div style={{ position: "fixed", inset: 0, background: GRAY100, zIndex: 300, overflowY: "auto" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "28px 16px 60px" }}>
        ...
        {!loading && submissions.length === 0 && (
          <Card><div style={{ textAlign: "center", padding: 20 }}><div style={{ fontSize: 40, marginBottom: 12 }}>📭</div><p style={{ color: GRAY600, fontSize: 14 }}>No submissions yet. They will appear here once Pros complete their posts on the live site.</p></div></Card>
        )}
...
function VoiceMode({ onComplete }) {
  ...
  const [coachMsg, setCoachMsg] = useState("Hey! I am going to walk you through a series of questions to build your story. Tap Start to begin.");
  ...
  useEffect(() => {
    ...
    r.onresult = async e => {
      ...
      setCoachMsg(msg); speakEL(msg); setAiThinking(false);
      if (isAccept) {
  ...
  const handleStart = () => { setVoiceStep("listening"); const msg = ALL_QUESTIONS[0].hint; setCoachMsg(msg); speakEL(msg); setTimeout(startListening, 1800); };
  const handleRerecord = id => { setRerecordId(id); const q = ALL_QUESTIONS.find(x => x.id === id); const msg = "Sure, let's redo that. " + q.hint; setCoachMsg(msg); speakEL(msg); setTranscript(""); setVoiceStep("rerecord"); if (!listening) startListening(); };
  useEffect(() => { if (voiceStep === "done") { const msg = "That is everything I need. Amazing work. Let's build your post now."; setCoachMsg(msg); speakEL(msg); } }, [voiceStep]);
          ...
          <>
            ...
            {watched && (
              <Card>
                ...
                <p style={{ color: GRAY600, fontSize: 14, lineHeight: 1.7, marginTop: 0, marginBottom: 20 }}>The longer you wait to follow up on a like, comment, share, or DM, the less likely it turns into a job. Every interaction is a chance to move the lead forward.</p>
                ...
                {[
                  ...
                  { step: "Move to DM",     example: "Hey, thanks so much for reaching out. I really appreciate it. I wasn't sure how that post would land, so it means a lot that it resonated. Just curious, what's been going on?", action: "Send a DM right after. Open the conversation - don't pitch yet." },
                  { step: "Book the job",   example: "I've got a few openings this week. Want me to save you a spot? I just need your name, address, email address, and phone number and I'll handle the rest.", action: "Once they describe their need, move to booking using this exact language." },
                ].map((s, i) => (
            ...
            {dmSent && (
              <Card>
                ...
                <p style={{ color: GRAY600, fontSize: 14, lineHeight: 1.7, marginTop: 0, marginBottom: 16 }}>You just moved from thinking about the system to actually using it. Most of your engagement will come in fast — in the first 2 to 3 days. Expand your reach while it is hot.</p>
                <div style={{ background: GRAY50, borderRadius: 12, padding: 16, marginBottom: 20 }}>
