...
const T = {
  ...
  es:{ voiceIntro:"Hola! Voy a guiarte por una serie de preguntas para construir tu historia.", voiceStart:"Iniciar Sesion de Voz", voiceDone:"Eso es todo. Excelente trabajo.", voiceContinue:"Continuar", listenHint:"Escuchando - di tu respuesta", processing:"Procesando...", answeredSoFar:"Tus Respuestas Hasta Ahora", rerecord:"Re-grabar", seeQuestion:"Ver pregunta actual" },
};
...
const HOME_T = {
  ...
  es:{ headline1:"Convierte Tu Historia en", headline2:"Trabajos en Tu Calendario", subheadline:"Construye una publicacion de Facebook que genera confianza.", stat1:"~29 min en total", stat2:"10 publicaciones", stat3:"Leads reales", continueLabel:"Continuar donde lo dejaste", cta0:"Empezar", cta1:"Generar Tu Publicacion", cta2:"Publicar en Mas Grupos", cta3:"Trabajar Tus Leads", checklistTitle:"Tu Lista de la Semana 1", writeTitle:"Escribir", writeDesc:"Responde 10 preguntas.", writeTime:"~8 min", groupsTitle:"Publicar en Grupos", groupsDesc:"Encuentra grupos y replica.", groupsTime:"~15 min", leadsTitle:"Trabajar Leads", leadsDesc:"Convierte cada engagement en un trabajo.", leadsTime:"~5 min", leadsProgress:"Scripts para cada tipo", statusNotStarted:"Sin Empezar", statusInProgress:"En Progreso", statusDone:"Listo", ofAnswered:"de 10 respondidas", ofGroups:"de 10 grupos", typeTitle:"Escribir Mis Respuestas", typeDesc:"Completa cada pregunta a tu ritmo.", voiceTitle:"Hablar con el Coach", voiceDesc:"Di tus respuestas en Espanol.", howStart:"Como quieres responder?", writePostTitle:"Escribe Tu Publicacion" },
};
...
function validateAnswer(q,text){
  const t=text.trim(); const words=t.split(/\s+/).filter(Boolean);
  ...
  if(q.validate==="name"){
    ...
    if(/\b(timer|alarm|minute|second|hey|hello|siri|google|alexa|cancel|stop|okay)\b/i.test(t)) return {ok:false,reason:"not_a_name"};
  }
  if(q.validate==="area"){
...
async function ttsSpeak(text,lang,onEnd,audioRef){
  ...
  if(window.speechSynthesis){
    ...
    const u=new SpeechSynthesisUtterance(text); u.lang=lang||"en-US"; u.rate=0.95; u.pitch=1.05;
    const safety=setTimeout(()=>{window.speechSynthesis.cancel();done();},Math.max(3000,text.length*65));
    u.onend=()=>{clearTimeout(safety);done();}; u.onerror=()=>{clearTimeout(safety);done();};
...
async function generateAIPost(ans){
  ...
  const p="You are a professional copywriter writing scroll-stopping neighborly Facebook posts for home service business owners.\n\n"
    +"STRICT NO-HALLUCINATION: Use ONLY facts explicitly stated below. Never invent names, places, years, prices, quotes, or outcomes.\n\n"
...
function SectionHeader({emoji,title,subtitle}){
  return(<div style={{marginBottom:20}}><div style={{fontSize:32,marginBottom:8}}>{emoji}</div><h2 style={{fontSize:22,fontWeight:800,color:NAVY,margin:0}}>{title}</h2>{subtitle&&<p style={{color:GRAY600,margin:"8px 0 0",fontSize:14,lineHeight:1.6}}>{subtitle}</p>}</div>);
}
function LangToggle({lang,setLang}){
  return(<div style={{display:"inline-flex",border:"2px solid "+NAVY,borderRadius:10,overflow:"hidden",marginBottom:20}}>{["en","es"].map(l=><button key={l} onClick={()=>setLang(l)} style={{background:lang===l?NAVY:WHITE,color:lang===l?YELLOW:NAVY,border:"none",padding:"7px 20px",fontWeight:700,fontSize:13,cursor:"pointer",transition:"all 0.2s"}}>{l==="en"?"English":"Espanol"}</button>)}</div>);
}
          ...
          return(
            <div key={section.id} style={{flex:1,minWidth:0}}>
              <div onClick={()=>onNavigate&&onNavigate(section.phases[0])}
                style={{background:sectionDone?GREEN:sectionActive?NAVY:GRAY200,color:sectionDone?WHITE:sectionActive?YELLOW:GRAY600,borderRadius:sectionActive&&!isWrite?"10px 10px 0 0":10,padding:"10px 14px",fontSize:13,fontWeight:800,textAlign:"center",cursor:"pointer",transition:"all 0.2s",userSelect:"none"}}>
              ...
              {sectionActive&&!isWrite&&(
                <div style={{background:GRAY50,border:"1px solid "+GRAY200,borderTop:"none",borderRadius:"0 0 10px 10px",padding:"8px 10px",display:"flex",flexDirection:"column",gap:5}}>
                  {subPhases.map((p,i)=>{
                    ...
                    return(<div key={p} onClick={e=>{e.stopPropagation();onNavigate&&onNavigate(p);}}
...
function VoiceMode({onComplete,lang}){
  ...
  function resumeSession(){setPaused(false);const q=S.current.rerecordId?ALL_QUESTIONS.find(x=>x.id===S.current.rerecordId):ALL_QUESTIONS[S.current.qIdx];const vq=q.voiceQ||q.hint;const msg=lang==="es"?"Bienvenido de vuelta. Continuemos. "+vq:"Welcome back! Let us pick up where we left off. "+vq;coachSay(msg,()=>openMic());}

  async function handleAnswer(text){
    const qIdx=S.current.qIdx; const rerecordId=S.current.rerecordId;
    ...
    if(!validation.ok){
      ...
      const existingPartial=S.current.partialAnswers[q.id]||"";
      if(isAdditive&&text.trim().length>3&&validation.reason!=="filler"&&validation.reason!=="not_a_name"){
        S.current.partialAnswers[q.id]=existingPartial?existingPartial+" "+text:text;
      }
      const templates=FOLLOWUP_TEMPLATES[q.id];
      let retryMsg="";
      if(q.id==="area"&&validation.reason==="missing_duration") retryMsg=templates.missing_duration[0];
      else if(validation.reason==="not_a_name"||validation.reason==="filler") retryMsg=Array.isArray(templates)?templates[0]:templates.default[0];
      else retryMsg=Array.isArray(templates)?templates[0]:templates.default?templates.default[0]:q.voiceQ;
      S.current.followupCount=(S.current.followupCount||0)+1;
      if(S.current.followupCount>=2){
        const combined=existingPartial?existingPartial+" "+text:text;
    ...
    const forcedAccept=S.current.forcedAccept;
    ...
    const sysPrompt="You are a warm conversational business coach. STRICT RULES:\n1. NEVER use filler words: Perfect, Love that, Great, Awesome, Got it, Amazing, Wonderful, Fantastic.\n2. React to ONE specific detail from their answer.\n3. Keep your response to 1-2 sentences maximum.\n4. After your reaction, ask EXACTLY this next question: "+(nextVoiceQ||"[end]")+"\n5. NEVER introduce new subjects.\n6. Start with ACCEPT then your message.";
    const userPrompt="They answered Q"+q.num+" ("+q.label+").\nAnswer: \""+finalText+"\"\n"+(isLast?"This is the last question. Start with ACCEPT then say: "+t.voiceDone:"Start with ACCEPT, react briefly, then ask: "+nextVoiceQ);
    let reply="";
    try{const timeout=new Promise((_,rej)=>setTimeout(()=>rej(new Error("timeout")),9000));reply=await Promise.race([callClaude([{role:"user",content:userPrompt}],sysPrompt),timeout]);}
  ...
  useEffect(()=>{
    ...
    r.onresult=e=>{const latest=e.results[e.results.length-1];const interimText=latest[0].transcript;setTranscript(interimText);if(!latest.isFinal)return;if(!interimText.trim()||S.current.busy)return;S.current.busy=true;S.current.wantMic=false;setUiStatus("thinking");handleAnswer(interimText.trim());};
    r.onerror=e=>{if(e.error==="no-speech"){if(S.current.wantMic&&!S.current.busy)try{r.start();}catch(_){}return;}S.current.wantMic=false;setMicErr(e.error==="not-allowed"?"Mic blocked - please allow microphone.":"Mic error: "+e.error);setUiStatus("idle");};
  ...
  function handleStart(){const first=ALL_QUESTIONS.findIndex(q=>!S.current.answers[q.id]);const idx=first===-1?0:first;const isResuming=idx>0;S.current.qIdx=idx;setDisplayQIdx(idx);S.current.followupCount=0;const q=ALL_QUESTIONS[idx];const firstQ=q.voiceQ||q.hint;const intro=isResuming?(lang==="es"?"Bienvenido de vuelta. "+firstQ:"Welcome back! "+firstQ):(lang==="es"?"Empecemos. "+firstQ:"Alright, let us build your story! Just talk to me like a friend. Here we go: "+firstQ);coachSay(intro,()=>openMic());}
  function handleRerecord(id){S.current.rerecordId=id;S.current.followupCount=0;const q=ALL_QUESTIONS.find(x=>x.id===id);const msg=lang==="es"?"Claro, repitamos. "+q.voiceQ:"No problem, let us redo that one. "+q.voiceQ;setTranscript("");coachSay(msg,()=>openMic());}
      ...
      {!paused&&uiStatus!=="idle"&&uiStatus!=="done"&&(
        <Card>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:14,paddingTop:8,paddingBottom:8}}>
            ...
            <span style={{fontSize:13,color:isListening?RED:isSpeaking?YELLOW:GRAY400,fontWeight:isListening||isSpeaking?600:400,textAlign:"center"}}>{isListening?t.listenHint:isThinking?t.processing:isSpeaking?(lang==="es"?"Coach hablando...":"Coach speaking..."):"Tap to start listening"}</span>
          ...
          {activeQ&&(<details style={{marginTop:8}}><summary style={{fontSize:12,color:GRAY400,cursor:"pointer",userSelect:"none"}}>{activeQ.chapter.toUpperCase()} - Q{activeQ.num} of {ALL_QUESTIONS.length} - <span style={{textDecoration:"underline"}}>{t.seeQuestion}</span></summary><div style={{marginTop:8,padding:"10px 14px",background:GRAY50,borderRadius:10,fontSize:13,color:GRAY600,lineHeight:1.6}}><strong style={{color:NAVY}}>{activeQ.label}</strong><br/>{activeQ.hint}</div></details>)}
        </Card>
      )}
...
function TypeMode({onComplete,lang,savedAnswers,onAnswerChange}){
  ...
  const handleInspiration=async()=>{
    ...
    try{const reply=await callClaude([{role:"user",content:"Give 3 short vivid example answers for this question from a home service business owner:\n\nQuestion: "+q.question+"\nContext: "+q.hint+"\n\nReturn ONLY a JSON array of 3 strings. Authentic and specific. No markdown, no code fences."}]);const cleaned=reply.replace(/```json/gi,"").replace(/```/g,"").trim();const s=cleaned.indexOf("[");const e=cleaned.lastIndexOf("]");if(s!==-1&&e!==-1){const p=JSON.parse(cleaned.slice(s,e+1));setInspirationExamples(Array.isArray(p)?p:[]);}}
  ...
  return(
    <div style={{paddingBottom:20}}>
      ...
      <div style={{background:WHITE,borderRadius:16,padding:28,boxShadow:"0 4px 24px rgba(0,41,66,0.08)",marginBottom:16}}>
        ...
        {q.missionChips&&(
          <div style={{marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {q.missionChips.map((chip,i)=>(<button key={i} onClick={()=>onAnswerChange(q.id,chip)} style={{background:value===chip?NAVY:GRAY50,color:value===chip?YELLOW:GRAY800,border:"2px solid "+(value===chip?NAVY:GRAY200),borderRadius:99,padding:"7px 16px",fontSize:13,fontWeight:700,cursor:"pointer",transition:"all 0.15s"}}>{value===chip?"✓ ":""}{chip}</button>))}
            </div>
          </div>
        )}

        <div style={{position:"relative"}}>
          <textarea value={value} onChange={e=>{onAnswerChange(q.id,e.target.value);if(showError)setShowError(false);}} placeholder={q.placeholder} rows={q.id==="heroMoment"?6:q.id==="whyStarted"||q.id==="fear"?4:3} autoFocus
          ...
          <button type="button" onClick={toggleMic} style={{position:"absolute",top:12,right:12,background:micListening?RED:GRAY100,border:"none",borderRadius:"50%",width:34,height:34,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:micListening?"0 0 0 5px rgba(239,68,68,0.2)":"none",transition:"all 0.2s"}}>
      ...
      {/* Inspiration below card */}
      <div style={{marginBottom:20}}>
        <button onClick={handleInspiration} style={{background:"transparent",border:"2px dashed "+GRAY300,borderRadius:10,padding:"9px 20px",fontSize:13,color:GRAY600,cursor:"pointer",fontWeight:600,display:"inline-flex",alignItems:"center",gap:6}}>
...
export default function App(){
  ...
  async function handleGeneratePost(ans){const a=ans||answers;setPostLoading(true);setPostError("");try{const generated=await generateAIPost(a);setPost(generated);setCompletedSections(prev=>prev.includes("write")?prev:[...prev,"write"]);await saveSubmission(a,generated,"Post Generated");}catch(e){setPostError("Could not generate post. Please check your connection and try again.");}setPostLoading(false);}
  ...
  return(
    <div style={{minHeight:"100vh",background:GRAY100,fontFamily:"'Inter', -apple-system, sans-serif"}}>
      ...
      <div ref={topRef} style={{maxWidth:960,margin:"0 auto",padding:"28px 32px 60px"}}>
          ...
          <>
            <Card>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12,marginTop:8}}>
                <button onClick={()=>setAppPhase("ch1")} style={{background:WHITE,border:"2px solid "+NAVY,borderRadius:14,padding:20,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:16}}><div style={{fontSize:32,flexShrink:0}}>⌨️</div><div><div style={{fontWeight:800,color:NAVY,fontSize:15,marginBottom:4}}>{ht.typeTitle}</div><div style={{fontSize:13,color:GRAY600,lineHeight:1.5}}>{ht.typeDesc}</div></div></button>
                <button onClick={()=>setAppPhase("voice")} style={{background:NAVY,border:"2px solid "+NAVY,borderRadius:14,padding:20,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:16}}><div style={{fontSize:32,flexShrink:0}}>🎤</div><div><div style={{fontWeight:800,color:YELLOW,fontSize:15,marginBottom:4}}>{ht.voiceTitle}</div><div style={{fontSize:13,color:GRAY400,lineHeight:1.5}}>{ht.voiceDesc}</div></div></button>
              </div>
            </Card>
        ...
        {/* GET & COPY POST */}
        {appPhase==="getpost"&&(
          <GetPost allCh3Met={allCh3Met} post={post} postLoading={postLoading} postError={postError} answers={answers} onGenerate={handleGeneratePost} onSetPost={setPost} onNext={()=>setAppPhase("photo")} onBack={()=>setAppPhase("groups")} onWritePost={()=>setAppPhase("writechoice")}/>
        )}
          ...
          <>
            <Card>
              ...
              <div style={{background:GRAY50,borderRadius:12,padding:16,marginBottom:20}}>
                {["Use the exact same post - do not change a single word.","Attach the exact same photo.","Do not add your phone number, website, or any contact info.","Public groups: post immediately. Private groups: join and post once approved."].map((s,i)=>(<div key={i} style={{display:"flex",gap:10,marginBottom:8,alignItems:"flex-start"}}><div style={{background:YELLOW,color:NAVY,borderRadius:99,width:22,height:22,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:11,flexShrink:0,marginTop:1}}>{i+1}</div><p style={{margin:0,fontSize:13,color:GRAY800,lineHeight:1.6}}>{s}</p></div>))}
              </div>
