...
const ALL_QUESTIONS = [
  ...
  {id:"refuses",      num:"5b", chapter:"ch1",minWords:15, label:"What You Refuse to Do",       hint:"One firm thing you refuse to do that others in your industry do. Minimum 15 words.",                            placeholder:"e.g. I will never tell someone they need a full repipe just to run up a bill when a simple repair will fix the problem"},
  ...
  {id:"whatChanged",  num:"9b", chapter:"ch3",minWords:10, label:"What Did It Change?",         hint:"A real concrete change in your life or family since going out on your own. Minimum 10 words.",                 placeholder:"e.g. Now I make it home for dinner most nights and I coached my son's baseball team"},
...
const LEAD_FUNNEL = [
  {emoji:"✉️",fires:"🔥🔥🔥🔥",type:"Direct Message",level:"High Intent",   desc:"They took initiative. Warmest lead you have. Respond within 24 hours."},
  {emoji:"💬",fires:"🔥🔥🔥",  type:"Comment",       level:"Medium-High", desc:"Starting a conversation in front of others. Reply publicly, then DM immediately."},
  {emoji:"🔁",fires:"🔥🔥",    type:"Share",         level:"Medium",      desc:"A referral without being asked. Comment on the share, then follow up with a DM."},
  {emoji:"👍",fires:"🔥",      type:"Like / Emoji",  level:"Low-Medium",  desc:"Lightest engagement but still interest. Send a DM within 24 hours."},
];
...
const T = {
  ...
  es: {
    voiceIntro: "¡Hola! Voy a guiarte por una serie de preguntas para construir tu historia. Toca Iniciar y te guiaré en todo.",
    voiceStart: "Iniciar Sesión de Voz",
    voiceDone: "Eso es todo lo que necesito. Excelente trabajo. Ahora construyamos tu publicación.",
    voiceContinue: "Continuar a Publicar en Grupos →",
    accept: "ACCEPT",
    followup: "FOLLOWUP",
...
const Q_ES = {
  name:         {label:"Tu Nombre",              hint:"Nombre y apellido. Mínimo 2 palabras."},
  business:     {label:"Nombre del Negocio",     hint:"Como aparece en línea. Mínimo 2 palabras."},
  area:         {label:"Dónde Trabajas",         hint:"Ciudades o vecindarios que atiendes y cuánto tiempo llevas. Mínimo 8 palabras."},
  knownFor:     {label:"Por qué te conocen",     hint:"2-3 palabras específicas. Sin palabras genéricas. Mínimo 3 palabras."},
  topServices:  {label:"Top 2 Servicios",        hint:"Los dos servicios que más quieres que te conozcan. Mínimo 4 palabras."},
  refuses:      {label:"Lo que te niegas a hacer",hint:"Una cosa firme que te niegas a hacer. Mínimo 15 palabras."},
  humanDetail:  {label:"Un Detalle Humano",      hint:"Un detalle real tuyo fuera del trabajo con detalles específicos. Mínimo 15 palabras."},
  localPlace:   {label:"Lugar Local",            hint:"Un lugar local específico que genuinamente amas. Mínimo 4 palabras."},
  localActivity:{label:"Qué Haces Allí",         hint:"Exactamente qué haces o siempre pides ahí. Mínimo 10 palabras."},
  mission:      {label:"Tu Misión",              hint:"Completa: Estoy en una misión para encontrar el mejor ___. Mínimo 3 palabras."},
  whyStarted:   {label:"Por qué Empezaste",      hint:"El momento real que te impulsó a comenzar. Mínimo 20 palabras."},
  whatChanged:  {label:"¿Qué Cambió?",           hint:"Un cambio real en tu vida o familia. Mínimo 10 palabras."},
  heroCrisis:   {label:"Momento Héroe - La Crisis",   hint:"Pinta el panorama completo de quién estaba en problemas. Mínimo 20 palabras."},
  heroSacrifice:{label:"Momento Héroe - El Sacrificio",hint:"Qué hiciste o sacrificaste. Mínimo 10 palabras."},
  heroPayoff:   {label:"Momento Héroe - El Resultado", hint:"La reacción emocional real. Mínimo 10 palabras."},
};
...
async function findFacebookGroups(city,count){
  const reply=await callClaude([{role:"user",content:"Generate "+count+" realistic Facebook group names for the "+city+" area that a home service contractor could post in.\n\nReturn ONLY a raw JSON array:\n[{\"name\": \"group name\", \"type\": \"Community or Homeowners or Family or Buy/Sell or Neighborhood\", \"members\": \"e.g. 4.2K\", \"privacy\": \"Public or Private\"}]"}]);
  const match=reply.match(/\[[\s\S]*?\]/);
  if(!match) return [];
  try{
    const parsed=JSON.parse(match[0]);
    return Array.isArray(parsed)?parsed.filter(g=>g.name).map(g=>({...g,url:"https://www.facebook.com/search/groups/?q="+encodeURIComponent(g.name)})):[];
  }catch(e){return [];}
}
...
function QField({q,value,onChange,aiFeedback,lang}){
  ...
  return(
    <div id={"qfield-"+q.id} style={{marginBottom:20,borderRadius:12,padding:aiFeedback?"14px 14px 4px":0,background:aiFeedback?"#FFF5F5":"transparent",border:aiFeedback?"2px solid #FCA5A5":"2px solid transparent"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
        ...
        <span style={{fontWeight:700,color:aiFeedback?RED:NAVY,fontSize:15}}>{label}</span>
        <MicBtn onTranscript={handleMic} size={28} lang={lang}/>
      </div>
      <p style={{fontSize:13,color:GRAY600,marginBottom:4,marginTop:0}}>{hint}</p>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if((appPhase==="replicate"||appPhase==="leads")&&groups20.length===0&&!groupsLoading){
  ...
  const handleCopy=()=>{if(post)navigator.clipboard.writeText(post).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2500);});};
      ...
      <div ref={topRef} style={{maxWidth:680,margin:"0 auto",padding:"28px 16px 60px"}}>
        ...
        {appPhase==="voice"&&(
          <>
            ...
            <VoiceMode onComplete={handleVoiceComplete} lang={lang}/>
          </>
        )}
        ...
        {appPhase==="ch1"&&(
          <Card>
            ...
            <div style={{height:16}}/>
            {ch1Qs.map(q=><QField key={q.id} q={q} value={answers[q.id]||""} onChange={setAnswer} aiFeedback={validationFeedback[q.id]} lang={lang}/>)}
        ...
        {appPhase==="ch2"&&(
          <Card>
            ...
            <div style={{height:16}}/>
            {ch2Qs.map(q=><QField key={q.id} q={q} value={answers[q.id]||""} onChange={setAnswer} aiFeedback={validationFeedback[q.id]} lang={lang}/>)}
            {Object.keys(validationFeedback).length>0&&<div style={{background:"#FEF2F2",borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:13,color:"#991B1B",fontWeight:600}}>⬆️ A few answers need more detail.</div>}
        ...
        {appPhase==="ch3"&&(
          <Card>
            ...
            <div style={{height:16}}/>
            {ch3Qs.map(q=><QField key={q.id} q={q} value={answers[q.id]||""} onChange={setAnswer} aiFeedback={validationFeedback[q.id]} lang={lang}/>)}
            {Object.keys(validationFeedback).length>0&&<div style={{background:"#FEF2F2",borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:13,color:"#991B1B",fontWeight:600}}>⬆️ A few answers need more detail.</div>}
            <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
          ...
          <>
            <Card>
              ...
              {!groupsLoading&&groups5.length>0&&(
                <>
                  <div style={{background:"#EFF6FF",borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:13,color:NAVY,lineHeight:1.6}}>💡 Click a group name to open Facebook. Find it at the top, click in, and join.</div>
            ...
            <Card>
              ...
              {ch3Met&&!post&&(
                <>
                  ...
                  {!postLoading&&!post&&<Btn onClick={()=>handleGeneratePost()}>Generate My Post →</Btn>}
                  {post&&!postLoading&&<Btn onClick={()=>setAppPhase("copypost")}>My Post Is Ready →</Btn>}
                </>
              )}
              {ch3Met&&postLoading&&<div style={{textAlign:"center",padding:40}}><div style={{fontSize:32,marginBottom:12}}>✨</div><p style={{color:GRAY600,fontSize:14}}>Writing your post...</p></div>}
              {ch3Met&&post&&!postLoading&&<Btn onClick={()=>setAppPhase("copypost")}>My Post Is Ready →</Btn>}
              {!ch3Met&&!post&&(
                <>
                  <div style={{background:"#FEF9EC",border:"1.5px solid "+YELLOW,borderRadius:10,padding:"16px",marginBottom:20}}>
                    <p style={{fontWeight:700,color:NAVY,fontSize:14,margin:"0 0 6px"}}>You'll need a post to continue.</p>
                    <p style={{color:GRAY600,fontSize:13,lineHeight:1.7,margin:"0 0 14px"}}>Go back to Write Post to build your story and generate one, or paste an existing post below.</p>
                    <Btn onClick={()=>setAppPhase("writechoice")}>← Write My Post</Btn>
                  </div>
                  <p style={{fontSize:13,fontWeight:600,color:NAVY,marginBottom:8}}>Or paste your existing post here:</p>
                  ...
                  {post&&post.trim().length>=50&&<div style={{marginTop:14}}><Btn onClick={()=>setAppPhase("copypost")}>Use This Post →</Btn></div>}
                </>
              )}
              {!ch3Met&&post&&<Btn onClick={()=>setAppPhase("copypost")}>My Post Is Ready →</Btn>}
            </Card>
          </>
        )}
        ...
        {appPhase==="copypost"&&(
          <>
            ...
            {copied&&<Btn onClick={()=>setAppPhase("photo")}>I Copied My Post →</Btn>}
          </>
        )}
        ...
        {appPhase==="photo"&&(
          <>
            <div style={{marginBottom:20}}><Btn variant="ghost" onClick={()=>setAppPhase("copypost")} style={{fontSize:13,padding:"8px 14px"}}>← Back</Btn></div>
            <Card>
        ...
        {appPhase==="leads"&&(
          <>
            <div style={{marginBottom:20}}><Btn variant="ghost" onClick={()=>setAppPhase("replicate")} style={{fontSize:13,padding:"8px 14px"}}>← Back</Btn></div>
            <Card>
              <SectionHeader emoji="🎉" title="Session Complete — Now We Execute" subtitle="You covered the Lead Engagement Playbook live with your Business Coach. Now we reinforce it."/>
              <TimeBadge phase="leads"/>
              <div style={{background:"#EFF6FF",borderRadius:12,padding:16}}>
                <p style={{margin:0,fontSize:14,color:NAVY,fontWeight:600}}>Step 1 — Watch the Training Video</p>
                <a href="https://tryhousecall-1.wistia.com/medias/indqjc1oov" target="_blank" rel="noopener noreferrer" style={{color:NAVY,fontSize:13,textDecoration:"underline",display:"block",marginTop:6}}>tryhousecall-1.wistia.com/medias/indqjc1oov</a>
                {!watched?<Btn style={{marginTop:12}} onClick={()=>setWatched(true)}>I watched the video ✓</Btn>:<p style={{color:GREEN,fontWeight:700,marginTop:8}}>✓ Video complete!</p>}
              </div>
            </Card>
            {watched&&(
              <Card>
                <h3 style={{color:NAVY,fontSize:18,margin:"0 0 4px"}}>⚡ Time Kills Deals</h3>
                <p style={{color:GRAY600,fontSize:14,lineHeight:1.7,marginTop:0,marginBottom:20}}>The longer you wait to follow up on a like, comment, share, or DM, the less likely it turns into a job.</p>
                <h3 style={{color:NAVY,fontSize:16,margin:"0 0 14px"}}>The Funnel — Every Interaction Leads to a DM</h3>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
                  {LEAD_FUNNEL.map((s,i)=>(
                    <div key={i} style={{background:GRAY50,borderRadius:12,padding:14}}>
                      <div style={{fontSize:24,marginBottom:4}}>{s.emoji}</div>
                      <div style={{fontWeight:800,color:NAVY,fontSize:13}}>{s.type}</div>
                      <div style={{fontSize:11,color:GRAY400,marginBottom:6}}>{s.fires} {s.level}</div>
                      <div style={{fontSize:12,color:GRAY600,lineHeight:1.5}}>{s.desc}</div>
                    </div>
                  ))}
                </div>
                <h3 style={{color:NAVY,fontSize:16,margin:"0 0 14px"}}>How a Comment Becomes a Booked Job</h3>
                {[
                  {step:"Reply publicly",example:"@[Name] great question. We do service [city name], we'd love to help you out.",action:"Reply using their @name. Keep it warm and brief."},
                  {step:"Move to DM",example:"Hey, thanks so much for reaching out. Just curious, what's been going on?",action:"Send a DM right after. Open the conversation — don't pitch yet."},
                  {step:"Book the job",example:"I've got a few openings this week. Want me to save you a spot? I just need your name, address, email, and phone number.",action:"Once they describe their need, move to booking using this exact language."},
                ].map((s,i)=>(
                  <div key={i} style={{marginBottom:18,borderLeft:"4px solid "+YELLOW,paddingLeft:16}}>
                    <div style={{fontWeight:800,color:NAVY,marginBottom:4}}>Step {i+1}: {s.step}</div>
                    <div style={{fontSize:13,color:GRAY600,fontStyle:"italic",marginBottom:6,background:GRAY50,padding:"8px 12px",borderRadius:8,lineHeight:1.6}}>{s.example}</div>
                    <div style={{fontSize:13,color:GRAY800}}>{s.action}</div>
                  </div>
                ))}
                <h3 style={{color:NAVY,fontSize:16,margin:"0 0 10px"}}>Like / Emoji DM Script</h3>
                <div style={{background:GRAY50,borderRadius:10,padding:"12px 16px",fontSize:13,color:GRAY800,lineHeight:1.8,fontStyle:"italic",marginBottom:20}}>
                  "Hey [Name], really appreciate the reaction on my post! The more people who engage, the more Facebook shows it around the community, so thank you for helping a small business like mine. Just curious, what stood out to you?"
                </div>
                <div style={{padding:16,background:NAVY,borderRadius:12,marginBottom:20}}>
                  ...
                  <p style={{color:WHITE,fontSize:13,margin:0}}>The first 24-48 hours matter most. Block time. Close the door. Work your leads.</p>
                </div>
                {!dmSent&&<Btn onClick={()=>setDmSent(true)}>I sent my first DM ✓</Btn>}
              </Card>
            )}
            {dmSent&&(
              <Card>
                <h3 style={{color:NAVY,fontSize:18,margin:"0 0 8px"}}>🔥 Now Keep the Momentum Going</h3>
                <p style={{color:GRAY600,fontSize:14,lineHeight:1.7,marginTop:0,marginBottom:16}}>Most of your engagement will come in fast — in the first 2 to 3 days. Expand your reach while it is hot.</p>
                <div style={{background:GRAY50,borderRadius:12,padding:16,marginBottom:20}}>
                  {["Same post. Same photo. No edits.","Join every relevant group you can find.","Public groups: post immediately. Private: join and post once approved.","Check back daily for likes, comments, and shares."].map((s,i)=>(
                    <div key={i} style={{display:"flex",gap:8,marginBottom:6}}>
                      <span style={{color:YELLOW,fontWeight:900,fontSize:16,lineHeight:1.3}}>→</span>
                      <span style={{fontSize:13,color:GRAY800,lineHeight:1.6}}>{s}</span>
                    </div>
                  ))}
                </div>
                {groups20.length>0&&<div style={{marginBottom:20}}><p style={{fontWeight:700,color:NAVY,fontSize:14,margin:"0 0 10px"}}>More groups to post in:</p><GroupTable groups={groups20} showNum={false}/></div>}
                {!stacked&&<Btn variant="success" onClick={()=>setStacked(true)}>I posted in more groups ✓</Btn>}
              </Card>
            )}
            {stacked&&(
              <Card style={{background:NAVY,textAlign:"center"}}>
                <div style={{fontSize:48,marginBottom:8}}>🔥</div>
                <h2 style={{color:YELLOW,fontSize:22,margin:"0 0 12px"}}>That is how you execute Week 1 at a high level.</h2>
                <p style={{color:WHITE,fontSize:14,lineHeight:1.8,margin:"0 0 16px"}}>You did not just hit 10 posts. You expanded. Most Pros stop at 10. You kept going.<br/><br/><strong style={{color:YELLOW}}>Visibility created the opportunity. Follow-up converts it.</strong></p>
                <div style={{background:NAVY_LIGHT,borderRadius:12,padding:16}}>
                  <p style={{color:YELLOW,fontWeight:700,margin:"0 0 6px"}}>Your only job for the next 48-72 hours:</p>
                  <p style={{color:WHITE,fontSize:13,margin:0,lineHeight:1.8}}>Reply to every like. Every comment. Every share. Every DM.<br/>Move every interaction into a DM. Use the Playbook scripts.<br/>Visibility created the opportunity. Follow-up converts it.</p>
                </div>
              </Card>
            )}
          </>
