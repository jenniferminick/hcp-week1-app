...
function CardNav({onBack,onNext,nextDisabled,nextLabel,firstQuestion}){return(<div style={{display:"flex",flexDirection:"row",justifyContent:firstQuestion||!onBack?"flex-end":"space-between",alignItems:"stretch",gap:16,marginTop:24,paddingTop:20,borderTop:"1px solid "+GRAY100}}>{onBack&&!firstQuestion&&<button onClick={onBack} style={{border:"none",borderRadius:10,fontWeight:700,fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",minHeight:56,padding:"0 28px",background:GRAY100,color:GRAY600}}>Back</button>}{onNext&&<button onClick={onNext} disabled={!!nextDisabled} style={{border:"none",borderRadius:10,fontWeight:700,fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",minHeight:56,padding:"0 28px",background:YELLOW,color:NAVY,opacity:nextDisabled?0.4:1}}>{nextLabel||"Next"}</button>}</div>);}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {groups.map((g,i)=>(
        <div key={i} style={{background:i%2===0?WHITE:GRAY50,border:"1px solid "+GRAY200,borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:NAVY,fontSize:14,marginBottom:2}}>{g.name}</div></div>
        ...
        {NAV_SECTIONS.map(section=>{
          ...
          return(
            <div key={section.id} style={{marginBottom:4}}>
              <button onClick={()=>onNavigate(section.phases[0])} style={{width:"100%",border:"none",background:"transparent",display:"flex",alignItems:"center",gap:10,padding:"11px 20px",cursor:"pointer",borderLeft:sectionActive?"3px solid "+YELLOW:"3px solid transparent"}}
                onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.05)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <span style={{fontSize:15,flexShrink:0,lineHeight:1}}>{section.icon}</span>
                <span style={{fontWeight:800,fontSize:13,textAlign:"left",flex:1,lineHeight:1.3,color:sectionDone?GREEN:sectionActive?YELLOW:GRAY400}}>{sectionDone?"✓ ":""}{section.label}</span>
              </button>
              {sectionActive&&!section.noSubs&&subPhases.length>1&&(
                <div style={{marginLeft:36,marginBottom:6,position:"relative"}}>
                  ...
                  {subPhases.map((p,i)=>{
                    ...
                    return(
                      <button key={p} onClick={()=>onNavigate(p)} style={{width:"100%",border:"none",background:isActive?"rgba(254,183,5,0.1)":"transparent",display:"flex",alignItems:"center",gap:10,padding:"7px 12px 7px 18px",cursor:"pointer",borderRadius:"0 8px 8px 0",position:"relative"}}
                        onMouseEnter={e=>{if(!isActive)e.currentTarget.style.background="rgba(255,255,255,0.05)";}} onMouseLeave={e=>{if(!isActive)e.currentTarget.style.background="transparent";}}>
                        ...
                        <span style={{fontSize:12,lineHeight:1.4,textAlign:"left",fontWeight:isActive?700:400,color:isActive?YELLOW:isDone?"#6EE7B7":GRAY400}}>{isDone?"✓ ":""}{PHASE_LABELS[p]||p}</span>
                      </button>
                    );
                  })}
                </div>
...
function VoiceMode({onComplete}){
  ...
  function coachSay(msg,after){S.current.busy=true;S.current.wantMic=false;closeMic();setCoachMsg(msg);setUiStatus("speaking");if(window.speechSynthesis){window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(msg);u.rate=0.95;u.pitch=1.05;const t=setTimeout(()=>{window.speechSynthesis.cancel();S.current.busy=false;if(after)after();else openMic();},Math.max(3000,msg.length*65));u.onend=()=>{clearTimeout(t);S.current.busy=false;if(after)after();else openMic();};u.onerror=()=>{clearTimeout(t);S.current.busy=false;if(after)after();else openMic();};window.speechSynthesis.speak(u);}else{setTimeout(()=>{S.current.busy=false;if(after)after();else openMic();},500);}}
  async function handleAnswer(text){
    ...
    if(!v.ok){
      ...
      const retry=q.id==="area"&&v.reason==="missing_state"?tmpl.missing_state[0]:q.id==="area"&&v.reason==="missing_duration"?tmpl.missing_duration[0]:(Array.isArray(tmpl)?tmpl[0]:(tmpl.default?tmpl.default[0]:q.voiceQ));
...
function TypeMode({onComplete,savedAnswers,onAnswerChange}){
  ...
  useEffect(()=>{
    // Reset inspiration when question changes
    ...
    r.onend=()=>setMicL(false);recogRef.current=r;
      ...
      <div style={{background:WHITE,borderRadius:16,padding:28,boxShadow:"0 4px 24px rgba(0,41,66,0.08)",marginBottom:16}}>
        ...
        {showInspiration&&(
          <div style={{marginBottom:14,background:"#FFFBEB",border:"1.5px solid "+YELLOW,borderRadius:12,padding:16}}>
            ...
            {inspirationLoading?<p style={{fontSize:13,color:GRAY400,fontStyle:"italic",margin:0}}>Loading examples...</p>:(inspirationExamples.length>0?inspirationExamples:q.examples||[]).map((ex,i,arr)=>(
  ...
  return(
    <Card>
      ...
      <div style={{marginBottom:28}}>
        ...
        {loading?<div style={{textAlign:"center",padding:24,color:GRAY400,fontSize:13}}>Generating personalized ideas...</div>:(
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(appPhase==="replicate"&&groups20.length===0&&!groupsLoading){setGroupsLoading(true);findFacebookGroups(city,10).then(r=>{setGroups20(r);setGroupsLoading(false);}).catch(()=>setGroupsLoading(false));}
  },[appPhase]);
    ...
    <div style={{minHeight:"100vh",background:GRAY100,fontFamily:"'Inter',-apple-system,sans-serif"}}>
      ...
      <div style={{background:NAVY,padding:"0 20px",height:HEADER_H,display:"flex",alignItems:"center",justifyContent:"space-between",position:"fixed",top:0,left:0,right:0,zIndex:160,boxShadow:"0 2px 12px rgba(0,0,0,0.2)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div><div style={{color:WHITE,fontWeight:800,fontSize:14,lineHeight:1}}>Business Coaching Foundations</div><div style={{color:YELLOW,fontSize:11,fontWeight:600}}>Week 1 — Facebook Organic Strategy</div></div>
        </div>
        ...
        <div ref={topRef} style={{maxWidth:680,margin:"0 auto",padding:"28px 16px 60px"}}>
          ...
          {appPhase==="lane"&&(
            <div>
              ...
              <button onClick={()=>{if(answeredN===0)setAppPhase("writechoice");else if(!post)setAppPhase("getpost");else setAppPhase("replicate");}} style={{width:"100%",background:YELLOW,border:"none",borderRadius:14,padding:"18px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",marginBottom:24,boxShadow:"0 4px 20px rgba(254,183,5,0.35)"}}>
          ...
          {appPhase==="groups"&&(
            <Card>
              ...
              {!groupsLoading&&groups5.length>0&&(<><div style={{background:"#EFF6FF",borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:13,color:NAVY,lineHeight:1.6}}>💡 These are search suggestions — each button opens a Facebook search for that group type in your area. Browse the results and join a public group.</div><GroupTable groups={groups5} showNum={false}/></>)}
