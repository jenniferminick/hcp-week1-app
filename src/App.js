...
const ALL_QUESTIONS = [
  ...
  { id:"area", num:3, chapter:"ch1", minWords:6, validate:"area", label:"Service Area", question:"What city, state/province, and surrounding areas do you serve — and how long have you been serving them?", hint:"Include your main city, state or province (e.g. TN, ON), and any nearby neighborhoods or towns. Then add how long you have been in the area.", examples:["Nashville, TN — Donelson, Mt. Juliet, Hermitage. 14 years.","Mesa, AZ — Gilbert, Chandler, Tempe. 8 years.","Toronto, ON — Scarborough, North York, Etobicoke. 6 years."], voiceQ:"What city and state or province do you serve, plus any surrounding areas — and how long have you been there?", placeholder:"e.g. Nashville, TN — Donelson, Mt. Juliet, Hermitage. 14 years." },
  { id:"fear", num:4, chapter:"ch1", minWords:12, validate:"fear", label:"Customer Fear You Fix", question:"Which homeowner fear do you focus on overcoming, and what is the one thing you do every job so customers never feel that fear?", hint:"Pick ONE fear, then give ONE habit you do every job.", examples:["Lack of communication. I send a quick update if anything changes so nobody is left wondering.","Getting overcharged. I show options and prices before I start and the final invoice always matches what we agreed to."], voiceQ:"Which homeowner fear do you focus on overcoming, and what is one specific thing you do every job so customers never feel that fear?", placeholder:"e.g. Lack of communication. I send a quick update any time something changes.", fearChips:["Getting overcharged","Being left with a mess","Lack of communication"] },
...
function validateAnswer(q, text) {
  ...
  if (q.validate==="name") { const nw=t.replace(/[^a-zA-Z\s'-]/g,"").trim().split(/\s+/).filter(Boolean); if(nw.length<2) return {ok:false,reason:"not_a_name"}; }
  if (q.validate==="area") { if(!/\b(\d+\s*(year|yr|years|month|months|decade)|for\s+\d+|going on\s+\d+|almost\s+\d+|over\s+\d+|nearly\s+\d+|new to|just moved|building our life|\d+\+?\s*year)/i.test(t)) return {ok:false,reason:"missing_duration"}; }
...
async function callClaude(messages, system) {
  ...
  // Use proxy on Vercel, direct API in Claude sandbox
...
function buildFacebookGroups(city, count) {
  ...
  return sfx.slice(0,count).map((s,i)=>({ name:city+" "+s, type:i<5?"Community":i<10?"Homeowners":"Neighborhood", privacy:i%3===1?"Private":"Public", members:(Math.floor(Math.random()*18)+1)+"."+(Math.floor(Math.random()*9))+"K", url:"https://www.facebook.com/groups/search/?q="+encodeURIComponent(city+" "+s) }));
}
...
async function generateAIPost(ans) {
  ...
  const prompt=["You are a professional copywriter writing scroll-stopping neighborly Facebook posts for home service business owners.","STRICT NO-HALLUCINATION: Use ONLY facts explicitly stated below.","Answers:","1. Name: "+(ans.name||""),"2. Business: "+(ans.business||""),"3. Area: "+(ans.area||""),"4. Customer fear: "+(ans.fear||""),"5. Real life detail: "+(ans.humanDetail||""),"6. Local flavor: "+(ans.localFlavor||""),"7. Mission: "+(ans.mission||""),"8. Why started: "+(ans.whyStarted||""),"9. What changed: "+(ans.whatChanged||""),"10. Hero moment: "+(ans.heroMoment||""),"RULES: First person only. Never use: professionalism, integrity, excellence, quality work, commitment, reliable, expert, top-notch. NEVER use em dashes or double hyphens. No CTA, phone, or website. Fix all grammar. 330-450 words. Short paragraphs.","STRUCTURE: (1) Vulnerable hook, (2) Identity anchor, (3) Fear + habit, (4) Hero moment as vivid scene, (5) Neighbor proof with real life detail + local flavor, (6) End ONLY with: Also, I am on a mission to find the best "+(ans.mission||"[mission]")+" in "+city+". Any suggestions?","Output post only. No labels."].join("\n");
...
function SidebarNav({ current, onNavigate, completedSections }) {
  ...
  return (
    <div style={{
      position:"fixed", top:HEADER_H, left:0, bottom:0,
      width:SIDEBAR_W,
      background:NAVY,
      zIndex:90,
      overflowY:"auto",
        ...
        {NAV_SECTIONS.map(section => {
          ...
          return (
            <div key={section.id} style={{ marginBottom:4 }}>
              {/* Section row */}
              <button
                onClick={() => onNavigate(section.phases[0])}
                style={{
                  width:"100%", border:"none", background:"transparent",
                  display:"flex", alignItems:"center", gap:10,
                  padding:"11px 20px",
                  cursor:"pointer",
                  borderLeft: sectionActive ? "3px solid "+YELLOW : "3px solid transparent",
                  transition:"background 0.15s",
                }}
                onMouseEnter={e => e.currentTarget.style.background="rgba(255,255,255,0.05)"}
                onMouseLeave={e => e.currentTarget.style.background="transparent"}
              >
                <span style={{ fontSize:15, flexShrink:0, lineHeight:1 }}>{section.icon}</span>
                <span style={{
                  fontWeight:800, fontSize:13, textAlign:"left", flex:1, lineHeight:1.3,
                  color: sectionDone ? GREEN : sectionActive ? YELLOW : GRAY400,
                }}>
                  {sectionDone ? "✓ " : ""}{section.label}
                </span>
              </button>

              {/* Sub-steps — only for active section with multiple phases, and not suppressed */}
              {sectionActive && subPhases.length > 1 && !section.noSubs && (
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          background: isActive ? "rgba(254,183,5,0.1)" : "transparent",
                          display:"flex", alignItems:"center", gap:10,
                      ...
                      >
                        ...
                        <span style={{
                          ...
                          background: isDone ? GREEN : isActive ? YELLOW : "#1e3a52",
                          border: isActive ? "2px solid "+YELLOW : isDone ? "2px solid "+GREEN : "2px solid rgba(255,255,255,0.15)",
                          boxSizing:"border-box",
                        }}/>
                        <span style={{
                          fontSize:12, lineHeight:1.4, textAlign:"left",
                          fontWeight: isActive ? 700 : 400,
                          color: isActive ? YELLOW : isDone ? "#6EE7B7" : GRAY400,
                        }}>
      ...
      {groups.map((g,i) => (
        <div key={i} style={{ background:i%2===0?WHITE:GRAY50, border:"1px solid "+GRAY200, borderRadius:12, padding:"12px 16px", display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontWeight:700, color:NAVY, fontSize:14, marginBottom:3 }}>{g.name}</div>
...
function VoiceMode({ onComplete }) {
  ...
  function coachSay(msg, after) {
    ...
    if(window.speechSynthesis){ window.speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(msg); u.rate=0.95; u.pitch=1.05; const t=setTimeout(()=>{window.speechSynthesis.cancel();S.current.busy=false;if(after)after();else openMic();},Math.max(3000,msg.length*65)); u.onend=()=>{clearTimeout(t);S.current.busy=false;if(after)after();else openMic();}; u.onerror=()=>{clearTimeout(t);S.current.busy=false;if(after)after();else openMic();}; window.speechSynthesis.speak(u); }
  ...
  async function handleAnswer(text) {
    ...
    if(!v.ok){
      ...
      const retry=q.id==="area"&&v.reason==="missing_duration"?tmpl.missing_duration[0]:q.id==="area"&&v.reason==="missing_state"?tmpl.missing_state[0]:(Array.isArray(tmpl)?tmpl[0]:(tmpl.default?tmpl.default[0]:q.voiceQ));
  ...
  return (
    <div>
      <Card style={{background:NAVY,marginBottom:16}}><div style={{display:"flex",gap:12,alignItems:"flex-start"}}><div style={{background:YELLOW,borderRadius:"50%",width:40,height:40,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontWeight:900,color:NAVY,fontSize:14}}>AI</div><div style={{flex:1}}>{isT?<p style={{color:GRAY400,fontSize:14,margin:0,fontStyle:"italic"}}>Processing...</p>:<p style={{color:WHITE,fontSize:16,margin:0,lineHeight:1.8}}>{coachMsg}</p>}</div></div></Card>
      {uiStatus!=="idle"&&uiStatus!=="done"&&(<Card><div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:14,paddingTop:8,paddingBottom:8}}><button type="button" onClick={isL?closeMic:openMic} disabled={isT||isS} style={{background:isL?RED:isS||isT?GRAY400:GRAY200,border:"none",borderRadius:"50%",width:80,height:80,cursor:isT||isS?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:isL?"0 0 0 10px rgba(239,68,68,0.15)":"none",transition:"all 0.3s"}}><svg width="32" height="32" viewBox="0 0 24 24" fill={isL?WHITE:GRAY600}><path d="M12 1a4 4 0 0 1 4 4v7a4 4 0 0 1-8 0V5a4 4 0 0 1 4-4zm-6 10a6 6 0 0 0 12 0h2a8 8 0 0 1-7 7.93V21h-2v-2.07A8 8 0 0 1 4 11h2z"/></svg></button><span style={{fontSize:13,color:isL?RED:isS?YELLOW:GRAY400,fontWeight:isL||isS?600:400}}>{isL?"Listening...":isT?"Processing...":isS?"Coach speaking...":"Tap to start"}</span>{micErr&&<span style={{fontSize:12,color:RED}}>{micErr}</span>}{transcript&&<div style={{background:"#F0F7FF",border:"2px solid "+NAVY,borderRadius:10,padding:"10px 14px",width:"100%",boxSizing:"border-box",fontSize:14,color:GRAY800,lineHeight:1.7}}><div style={{fontSize:11,color:GRAY400,marginBottom:4,fontWeight:600}}>YOU SAID:</div>{transcript}</div>}</div>{activeQ&&<div style={{marginTop:12,padding:"12px 14px",background:GRAY50,borderRadius:10}}><span style={{background:NAVY,color:YELLOW,borderRadius:6,padding:"1px 6px",fontSize:11,fontWeight:700,marginRight:6}}>Q{activeQ.num}</span><span style={{fontWeight:700,color:NAVY,fontSize:13}}>{activeQ.question}</span></div>}</Card>)}
      ...
      {answered>0&&(<Card><h3 style={{color:NAVY,margin:"0 0 16px",fontSize:16}}>📝 Your Answers So Far</h3>{ALL_QUESTIONS.filter(q=>displayAnswers[q.id]).map(q=>(<div key={q.id} style={{marginBottom:14,paddingBottom:14,borderBottom:"1px solid "+GRAY200}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}><div style={{flex:1}}><div style={{display:"flex",gap:6,alignItems:"center",marginBottom:4}}><span style={{background:NAVY,color:YELLOW,borderRadius:6,padding:"1px 6px",fontSize:11,fontWeight:700}}>Q{q.num}</span><span style={{fontWeight:700,color:NAVY,fontSize:13}}>{q.question}</span></div><p style={{margin:0,fontSize:13,color:GRAY800,lineHeight:1.6}}>{displayAnswers[q.id]}</p></div><button onClick={()=>{S.current.rerecordId=q.id;const rq=ALL_QUESTIONS.find(x=>x.id===q.id);setTranscript("");coachSay("No problem, let us redo that. "+(rq.voiceQ||rq.hint),()=>openMic());}} style={{background:GRAY100,border:"none",borderRadius:8,padding:"4px 10px",fontSize:12,color:GRAY600,cursor:"pointer",flexShrink:0}}>Re-record</button></div></div>))}</Card>)}
...
function GetPost({ allCh3Met, post, postLoading, postError, answers, onGenerate, onSetPost, onNext, onBack, onWritePost }) {
  ...
  useEffect(()=>{if(!taRef.current||!post)return;taRef.current.style.height="auto";requestAnimationFrame(()=>{if(taRef.current)taRef.current.style.height=taRef.current.scrollHeight+"px";});},[post]);
  const handleCopy=()=>{if(!post)return;try{const el=document.createElement("textarea");el.value=post;el.style.position="fixed";el.style.opacity="0";document.body.appendChild(el);el.select();document.execCommand("copy");document.body.removeChild(el);}catch(e){navigator.clipboard&&navigator.clipboard.writeText(post);}setCopied(true);setEverCopied(true);};
  useEffect(()=>{if(allCh3Met&&!post&&!postLoading)onGenerate(answers);},[]);
  return (
    <Card>
      <SectionHeader emoji="✍️" title="Step 2 — Generate Post"/>
      {!allCh3Met&&!post&&(<div style={{background:"#FEF9EC",border:"1.5px solid "+YELLOW,borderRadius:10,padding:16,marginBottom:20}}><p style={{fontWeight:700,color:NAVY,fontSize:14,margin:"0 0 6px"}}>You will need a post to continue.</p><p style={{color:GRAY600,fontSize:13,lineHeight:1.7,margin:"0 0 14px"}}>Go back to Write Post to build your story, or paste an existing post below.</p><Btn onClick={onWritePost}>Write My Post</Btn></div>)}
      ...
      {post&&!postLoading&&(<>          <textarea ref={taRef} value={post} onChange={e=>{onSetPost(e.target.value);setCopied(false);setEverCopied(false);e.target.style.height="auto";e.target.style.height=e.target.scrollHeight+"px";}} style={{width:"100%",boxSizing:"border-box",border:"2px solid "+NAVY,borderRadius:12,padding:16,fontSize:14,lineHeight:1.8,color:GRAY800,fontFamily:"inherit",resize:"none",background:GRAY50,outline:"none",marginBottom:14,overflow:"hidden",display:"block"}}/>          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",marginBottom:20}}><Btn onClick={handleCopy} variant={copied?"success":"primary"}>{copied?"Copied!":"Copy Post"}</Btn><button onClick={()=>{setCopied(false);setEverCopied(false);onGenerate(answers);}} disabled={postLoading} style={{background:"none",border:"2px solid "+GRAY300,borderRadius:10,padding:"10px 18px",fontSize:13,fontWeight:700,color:GRAY600,cursor:"pointer"}}>Regenerate</button></div></>)}
...
function TypeMode({ onComplete, savedAnswers, onAnswerChange }) {
  ...
  useEffect(()=>{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){setMicErr("Speech recognition not supported.");return;}const r=new SR();r.continuous=false;r.interimResults=false;r.lang="en-US";r.onresult=e=>{const t=e.results[0][0].transcript;const c=savedAnswers[q.id]||"";onAnswerChange(q.id,c?c+" "+t:t);setMicL(false);};r.onerror=e=>{setMicL(false);setMicErr(e.error==="not-allowed"?"Mic blocked.":"Mic error.");};r.onend=()=>setMicL(false);recogRef.current=r;return()=>{try{r.stop();}catch(_){}};},[ qIdx]);
  ...
  return (
    <div style={{paddingBottom:20}}>
      <div style={{background:WHITE,borderRadius:16,padding:"20px 24px",marginBottom:20,boxShadow:"0 4px 24px rgba(0,41,66,0.08)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{fontWeight:800,color:NAVY,fontSize:15}}>Your Story Progress</span><span style={{fontWeight:800,fontSize:14,color:pct===100?GREEN:NAVY}}>{answered} / {ALL_QUESTIONS.length}</span></div>
        <div style={{background:GRAY100,borderRadius:99,height:14,overflow:"hidden",position:"relative"}}><div style={{background:pct===100?GREEN:YELLOW,borderRadius:99,height:"100%",width:pct+"%",transition:"width 0.5s ease",position:"relative"}}>{pct>8&&<span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:10,fontWeight:800,color:NAVY}}>{pct}%</span>}</div></div>
      </div>
      <div style={{background:WHITE,borderRadius:16,padding:28,boxShadow:"0 4px 24px rgba(0,41,66,0.08)",marginBottom:16}}>
        ...
        {q.examples&&q.examples.length>0&&(<div style={{background:"#FFFBEB",border:"1.5px solid "+YELLOW,borderRadius:12,padding:14,marginBottom:16}}><p style={{fontSize:11,fontWeight:700,color:GRAY600,margin:"0 0 8px",textTransform:"uppercase",letterSpacing:"0.06em"}}>Examples</p>{q.examples.map((ex,i)=><p key={i} style={{fontSize:13,color:GRAY800,margin:"0 0 4px",lineHeight:1.7,fontStyle:"italic"}}>"{ex}"</p>)}</div>)}
        {q.fearChips&&(<div style={{marginBottom:16}}><p style={{fontSize:11,fontWeight:700,color:GRAY400,margin:"0 0 8px",textTransform:"uppercase",letterSpacing:"0.06em"}}>Pick the fear you focus on</p><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{q.fearChips.map((chip,i)=>{const sel=value.toLowerCase().startsWith(chip.toLowerCase());return(<button key={i} onClick={()=>{if(sel){onAnswerChange(q.id,value.replace(new RegExp("^"+chip+"\\.?\\s*","i"),"").trim());}else{onAnswerChange(q.id,chip+". "+value.replace(new RegExp("^(Getting overcharged|Being left with a mess|Lack of communication)\\.?\\s*","i"),"").trim());}}} style={{background:sel?NAVY:GRAY50,color:sel?YELLOW:GRAY800,border:"2px solid "+(sel?NAVY:GRAY200),borderRadius:99,padding:"7px 16px",fontSize:13,fontWeight:700,cursor:"pointer"}}>{sel?"✓ ":""}{chip}</button>);})}</div></div>)}
        {q.missionChips&&(<div style={{marginBottom:16}}><p style={{fontSize:11,fontWeight:700,color:GRAY400,margin:"0 0 8px",textTransform:"uppercase",letterSpacing:"0.06em"}}>Quick pick</p><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{q.missionChips.map((chip,i)=>(<button key={i} onClick={()=>onAnswerChange(q.id,value===chip?"":chip)} style={{background:value===chip?NAVY:GRAY50,color:value===chip?YELLOW:GRAY800,border:"2px solid "+(value===chip?NAVY:GRAY200),borderRadius:99,padding:"7px 16px",fontSize:13,fontWeight:700,cursor:"pointer"}}>{value===chip?"✓ ":""}{chip}</button>))}</div></div>)}
        <div style={{position:"relative"}}>
          ...
          <button type="button" onClick={toggleMic} style={{position:"absolute",top:12,right:12,background:micL?RED:GRAY100,border:"none",borderRadius:"50%",width:34,height:34,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="15" height="15" viewBox="0 0 24 24" fill={micL?WHITE:GRAY600}><path d="M12 1a4 4 0 0 1 4 4v7a4 4 0 0 1-8 0V5a4 4 0 0 1 4-4zm-6 10a6 6 0 0 0 12 0h2a8 8 0 0 1-7 7.93V21h-2v-2.07A8 8 0 0 1 4 11h2z"/></svg></button>
        </div>
...
function LeadEngagement({ onBack, onAmplify }) {
  ...
  if(!watched){return(<Card><SectionHeader emoji="🎉" title="Session Complete — Now We Execute" subtitle="Watch this training video before working your leads."/><div style={{background:"#EFF6FF",borderRadius:12,padding:16,marginBottom:16}}><p style={{margin:"0 0 12px",fontSize:14,color:NAVY,fontWeight:600}}>Lead Engagement Training Video</p><div style={{position:"relative",paddingBottom:"56.25%",height:0,borderRadius:10,overflow:"hidden",marginBottom:14}}><iframe src="https://fast.wistia.net/embed/iframe/indqjc1oov?autoPlay=false" title="Lead Engagement Training" allowFullScreen style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",border:"none",borderRadius:10}}/></div></div><CardNav onBack={onBack} onNext={()=>setWatched(true)} nextLabel="Next"/></Card>);}
  return(<>
    <Card style={{background:NAVY,marginBottom:16}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:12}}><div><h2 style={{color:YELLOW,fontSize:20,fontWeight:900,margin:"0 0 4px"}}>Work Your Leads</h2><p style={{color:GRAY400,fontSize:13,margin:0}}>Pick the type of engagement. Follow the steps. Book the job.</p></div><div style={{display:"flex",gap:10}}><div style={{background:"rgba(255,255,255,0.08)",borderRadius:12,padding:"10px 16px",textAlign:"center",minWidth:72}}><div style={{color:YELLOW,fontWeight:900,fontSize:24,lineHeight:1}}>{sessionTotal}</div><div style={{color:GRAY400,fontSize:10,marginTop:3}}>this session</div></div><div style={{background:"rgba(16,185,129,0.15)",border:"1px solid rgba(16,185,129,0.3)",borderRadius:12,padding:"10px 16px",textAlign:"center",minWidth:72}}><div style={{color:GREEN,fontWeight:900,fontSize:24,lineHeight:1}}>{totalJobs}</div><div style={{color:GREEN,fontSize:10,marginTop:3}}>jobs booked</div></div></div></div>
      <div style={{marginTop:14,padding:"14px 16px",background:"rgba(255,255,255,0.08)",borderRadius:10,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}><span style={{color:WHITE,fontSize:13,fontWeight:600}}>+ Log a booked job:</span><input type="number" min="1" value={jobsInput} onChange={e=>setJobsInput(e.target.value)} placeholder="# of jobs" style={{width:90,border:"2px solid rgba(255,255,255,0.3)",borderRadius:8,padding:"7px 10px",fontSize:15,fontWeight:800,color:NAVY,outline:"none",textAlign:"center",background:WHITE}}/><Btn onClick={()=>{const n=parseInt(jobsInput);if(n>0){setTotalJobs(j=>j+n);setJobsInput("");}}} disabled={!jobsInput||parseInt(jobsInput)<1} style={{fontSize:13}}>Add Jobs</Btn></div>
    ...
    {activeLead&&!activeLead.simple&&activeSubtype&&(<Card><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}><button onClick={()=>setSubtype(null)} style={{background:GRAY100,border:"none",borderRadius:8,padding:"6px 12px",fontSize:12,color:GRAY600,fontWeight:600,cursor:"pointer"}}>← Back</button><span style={{fontSize:26}}>{activeLead.emoji}</span><div><div style={{fontWeight:800,color:NAVY,fontSize:15}}>{activeLead.label}</div><div style={{fontSize:12,color:GRAY600}}>{activeSubtype.label}</div></div></div>{activeLead.id==="comment"&&(<div><div style={{fontWeight:700,color:NAVY,fontSize:14,marginBottom:8}}>1. Reply publicly</div>{activeSubtype.publicReplies.map((r,i)=><ScriptBox key={i} text={r} idx={i}/>)}<div style={{fontWeight:700,color:NAVY,fontSize:14,margin:"16px 0 8px"}}>2. Then immediately DM</div><ScriptBox text={activeSubtype.dmScript} idx={99}/></div>)}{activeLead.id==="dm"&&(<div><div style={{fontWeight:700,color:NAVY,fontSize:14,marginBottom:8}}>1. Reply within 24 hours</div><ScriptBox text={activeSubtype.dmScript} idx={0}/></div>)}<div style={{borderTop:"1px solid "+GRAY200,paddingTop:16,marginTop:16,display:"flex",gap:10}}><Btn variant="success" onClick={()=>{logLead(activeLead.id);reset();}}>Mark as Worked</Btn><Btn variant="ghost" onClick={()=>setSubtype(null)}>Different Response</Btn></div></Card>)}
    ...
    {!active&&(<Card style={{background:"linear-gradient(135deg,#002942,#003a5c)",textAlign:"center"}}><div style={{fontSize:36,marginBottom:8}}>⚡</div><h3 style={{color:YELLOW,fontSize:18,fontWeight:900,margin:"0 0 8px"}}>Keep the momentum going</h3><p style={{color:GRAY400,fontSize:13,lineHeight:1.7,margin:"0 0 20px"}}>Done working this batch? Head to Amplify to post in more groups.</p><div style={{display:"flex",gap:12,justifyContent:"space-between"}}><Btn variant="ghost" onClick={onBack} style={{background:"rgba(255,255,255,0.1)",color:WHITE}}>Back</Btn><Btn onClick={onAmplify}>Go to Amplify</Btn></div></Card>)}
  </>);
}
...
function AmplifyScreen({ onBack, city }) {
  ...
  useEffect(()=>{findFacebookGroups(city||"Your City",20).then(r=>{setGroups(r);setLoading(false);}).catch(()=>setLoading(false));},[city]);
  return(<><Card style={{background:NAVY}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}><div><h2 style={{color:YELLOW,fontSize:20,fontWeight:900,margin:"0 0 4px"}}>Amplify</h2><p style={{color:GRAY400,fontSize:13,margin:0}}>Keep the momentum going.</p></div><div style={{background:"rgba(255,255,255,0.08)",borderRadius:12,padding:"14px 20px",textAlign:"center"}}><div style={{color:YELLOW,fontWeight:900,fontSize:32,lineHeight:1}}>{10+extra}</div><div style={{color:GRAY400,fontSize:11,marginTop:4}}>total groups posted</div></div></div></Card><Card><Btn variant="success" onClick={()=>setExtra(n=>n+1)}>+ Posted in Another Group</Btn></Card><Card><h3 style={{color:NAVY,fontSize:16,fontWeight:800,margin:"0 0 16px"}}>More groups near you</h3>{loading&&<p style={{color:GRAY600,fontSize:14}}>Finding groups near {city}...</p>}{!loading&&groups.length>0&&<GroupTable groups={groups} showNum={false}/>}</Card><div style={{marginTop:8}}><Btn variant="ghost" onClick={onBack}>← Back</Btn></div></>);
}
...
function CoachDashboard({ onClose }) {
  ...
  async function load(){setLoading(true);try{const r=await window.storage.list("submission:",true);const keys=(r&&r.keys)?r.keys:[];const items=[];for(let i=0;i<keys.length;i++){try{const x=await window.storage.get(keys[i],true);if(x&&x.value)items.push(JSON.parse(x.value));}catch(e){}}setSubs(items.sort((a,b)=>b.timestamp-a.timestamp));}catch(e){setSubs([]);}setLoading(false);}
  useEffect(()=>{load();},[]);
  return(<div style={{position:"fixed",inset:0,background:GRAY100,zIndex:300,overflowY:"auto"}}><div style={{background:NAVY,padding:"16px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10,boxShadow:"0 2px 12px rgba(0,0,0,0.2)"}}><div style={{display:"flex",alignItems:"center",gap:14}}><div style={{background:YELLOW,borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,color:NAVY,fontSize:18}}>H</div><div><div style={{color:WHITE,fontWeight:800,fontSize:16}}>Coach Dashboard</div><div style={{color:YELLOW,fontSize:12,fontWeight:600}}>{subs.length} submissions</div></div></div><div style={{display:"flex",gap:8}}><button onClick={load} style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:8,padding:"6px 14px",color:WHITE,fontSize:12,fontWeight:600,cursor:"pointer"}}>Refresh</button><button onClick={onClose} style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:8,padding:"6px 14px",color:WHITE,fontSize:12,fontWeight:600,cursor:"pointer"}}>Back</button></div></div><div style={{maxWidth:800,margin:"0 auto",padding:"28px 16px 60px"}}>{loading&&<Card><p style={{textAlign:"center",color:GRAY600,padding:40}}>Loading...</p></Card>}{!loading&&subs.length===0&&<Card><div style={{textAlign:"center",padding:20}}><div style={{fontSize:40,marginBottom:12}}>📭</div><p style={{color:GRAY600}}>No submissions yet.</p></div></Card>}{!loading&&subs.map((s,i)=>(<Card key={i}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap"}}><div><div style={{fontWeight:800,color:NAVY,fontSize:17}}>{s.name||"Unknown"}</div><div style={{fontSize:13,color:GRAY600,marginTop:2}}>{s.business} — {s.city}</div><div style={{fontSize:12,color:GRAY400,marginTop:4}}>{s.timestamp?new Date(s.timestamp).toLocaleString():"—"}</div></div><span style={{background:"#DBEAFE",color:"#1D4ED8",borderRadius:99,padding:"3px 10px",fontSize:11,fontWeight:700}}>{s.status||"Post Generated"}</span></div><button onClick={()=>setExpanded(expanded===i?null:i)} style={{marginTop:14,background:GRAY50,border:"1px solid "+GRAY200,borderRadius:8,padding:"6px 14px",fontSize:12,color:NAVY,fontWeight:600,cursor:"pointer"}}>{expanded===i?"Hide Details ▲":"View Details ▼"}</button>{expanded===i&&s.post&&<div style={{marginTop:16}}><div style={{fontSize:13,fontWeight:700,color:NAVY,marginBottom:8}}>Generated Post:</div><div style={{background:GRAY50,border:"1px solid "+GRAY200,borderRadius:10,padding:16,fontSize:13,color:GRAY800,lineHeight:1.8,whiteSpace:"pre-wrap"}}>{s.post}</div></div>}</Card>))}</div></div>);
}

// ── Celebration ───────────────────────────────────────────────────────────────
function CelebrationScreen({ onNext, onBack }) {
  const msgs=[{h:"🏆 Achievement Unlocked!",s:"10 groups. 1 post. Zero sales pitches. Let's go!"},{h:"🎯 Mission Complete!",s:"You showed up as a neighbor. That's the whole game."},{h:"🔥 Week 1 = Done!",s:"Anti-Ad strategy executed. Leads incoming."}];
  const [msg]=useState(()=>msgs[Math.floor(Math.random()*msgs.length)]);
  return(<div><div style={{background:"linear-gradient(135deg,#002942,#003a5c)",borderRadius:20,padding:"40px 32px",marginBottom:20,textAlign:"center"}}><div style={{fontSize:72,marginBottom:16,lineHeight:1}}>🎯</div><h1 style={{color:YELLOW,fontSize:28,fontWeight:900,margin:"0 0 12px"}}>{msg.h}</h1><p style={{color:WHITE,fontSize:18,margin:0,fontWeight:600}}>{msg.s}</p></div><Card><h3 style={{color:NAVY,fontSize:17,fontWeight:800,margin:"0 0 16px"}}>Here is what you executed this week:</h3>{[{emoji:"👥",title:"Joined 10 local Facebook groups",desc:"You are now inside the conversations happening in your city."},{emoji:"✍️",title:"Created a trust-first intro post",desc:"No sales pitch. No contact info. Just your story."},{emoji:"📣",title:"Posted across 10 groups",desc:"Your face and your story are now in front of thousands of local homeowners."},{emoji:"👤",title:"Posted as a neighbor, not a business",desc:"Personal account. Real story. That is exactly why this works."}].map((item,i)=>(<div key={i} style={{display:"flex",gap:14,alignItems:"flex-start",background:GRAY50,borderRadius:12,padding:"14px 16px",marginBottom:10}}><span style={{fontSize:24,flexShrink:0}}>{item.emoji}</span><div><div style={{fontWeight:800,color:NAVY,fontSize:14,marginBottom:3}}>{item.title}</div><div style={{fontSize:13,color:GRAY600,lineHeight:1.5}}>{item.desc}</div></div></div>))}        <div style={{background:NAVY,borderRadius:14,padding:20,marginBottom:24,marginTop:8}}><p style={{color:YELLOW,fontWeight:900,fontSize:15,margin:"0 0 8px"}}>You joined the groups. You wrote the post. You executed the Anti-Ad strategy.</p><p style={{color:WHITE,fontSize:14,margin:0,lineHeight:1.8}}>You showed up as a neighbor, not a salesperson. Your leads are already coming in.</p></div>        <CardNav onBack={onBack} onNext={onNext} nextLabel="Next"/></Card></div>);
}
...
function ApprovalStep({ onBack, onNext }) {
  return (
    <Card>
  ...
  return (
    <div style={{ border:"1.5px solid "+GRAY200, borderRadius:12, overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"14px 16px", background:WHITE }}>
        <div style={{ background:GRAY100, borderRadius:6, width:22, height:22, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginTop:1 }}>
          <svg width="11" height="11" viewBox="0 0 12 12" fill="none"><rect x="1" y="1" width="10" height="10" rx="2" stroke={GRAY400} strokeWidth="1.5"/></svg>
        </div>
        <div style={{ flex:1 }}>
          <div style={{ fontWeight:700, color:NAVY, fontSize:14 }}>{item.label}</div>
          <div style={{ display:"flex", gap:16, marginTop:8, flexWrap:"wrap" }}>
            <div style={{ display:"flex", gap:6, alignItems:"flex-start" }}>
              <span style={{ background:"#D1FAE5", color:"#065F46", borderRadius:4, padding:"1px 6px", fontSize:10, fontWeight:800, flexShrink:0, marginTop:1 }}>DO</span>
              <span style={{ fontSize:12, color:GRAY600, lineHeight:1.5 }}>{item.doThis}</span>
            </div>
            <div style={{ display:"flex", gap:6, alignItems:"flex-start" }}>
              <span style={{ background:"#FEE2E2", color:"#991B1B", borderRadius:4, padding:"1px 6px", fontSize:10, fontWeight:800, flexShrink:0, marginTop:1 }}>NOT</span>
              <span style={{ fontSize:12, color:GRAY600, lineHeight:1.5 }}>{item.notThat}</span>
            </div>
          </div>
        </div>
        <button onClick={() => setOpen(o => !o)} style={{ background:"none", border:"none", cursor:"pointer", color:GRAY400, fontSize:18, lineHeight:1, flexShrink:0, padding:"0 2px" }} title="Why this matters">
          {open ? "▲" : "▼"}
        </button>
      </div>
      {/* Expandable tip */}
      {open && (
        <div style={{ background:"#FFFBEB", borderTop:"1px solid "+GRAY200, padding:"12px 16px", display:"flex", gap:10, alignItems:"flex-start" }}>
...
function PhotoStep({ answers, onBack, onNext }) {
  ...
  useEffect(() => {
    async function generate() {
      setLoading(true);
      try {
        const prompt = `A home service business owner filled out these details about themselves:
- Human detail outside work: ${answers.humanDetail || "not provided"}
- Local place they love: ${answers.localFlavor || "not provided"}
- Why they started: ${answers.whyStarted || "not provided"}
- Hero moment: ${answers.heroMoment || "not provided"}

Give them 3 personalized Facebook photo ideas rated Good, Better, and Best. Each should feel specific to who they are based on the details above.

Return ONLY a valid JSON object, no markdown:
{
  "good": {"label": "short title", "desc": "1 sentence why this works"},
  "better": {"label": "short title", "desc": "1 sentence why this works, tied to their details"},
  "best": {"label": "short title", "desc": "1 sentence why this is the most trust-building option for them specifically"}
}`;
        ...
        setPhotoIdeas({
          good:   { label:"Any real photo of you", desc:"Face visible, natural light, looks like a real person." },
          better: { label:"You doing something you love", desc:"A hobby, your kids' game, your local spot — something neighbors relate to." },
          best:   { label:"A candid moment on the job", desc:"You working, smiling, face clearly visible. Shows who you are and what you do." },
        });
      }
      setLoading(false);
    }
  ...
  const tiers = [
    { key:"good",   label:"Good",   color:"#F0FDF4", border:"#86EFAC", textColor:"#166534", badge:"GOOD" },
    { key:"better", label:"Better", color:"#EFF6FF", border:"#93C5FD", textColor:"#1E40AF", badge:"BETTER" },
    { key:"best",   label:"Best",   color:"#FEF9EC", border:YELLOW,    textColor:"#92400E", badge:"BEST ⭐" },
  ];
    ...
    <Card>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:10, marginBottom:20 }}>
          {GOOD_PHOTOS.map((src, i) => (
            <div key={i} style={{ borderRadius:10, overflow:"hidden", border:"2px solid #86EFAC", aspectRatio:"1", background:GRAY100 }}>
              <img src={src} alt="Good photo example" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }}/>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:10 }}>
          {BAD_PHOTOS.map((src, i) => (
            <div key={i} style={{ borderRadius:10, overflow:"hidden", border:"2px solid #FCA5A5", aspectRatio:"1", background:GRAY100 }}>
              <img src={src} alt="Bad photo example" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }}/>
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    if (appPhase==="groups" && groups5.length===0 && !groupsLoading) { setGroupsLoading(true); setGroupsError(""); findFacebookGroups(city,5).then(r=>{setGroups5(r);setGroupsLoading(false);}).catch(()=>{setGroupsLoading(false);setGroupsError("Search failed.");}); }
  ...
  async function saveSubmission(a, gp, status) {
    ...
    try { let prev={}; try{const ex=await window.storage.get(key,true);if(ex&&ex.value)prev=JSON.parse(ex.value);}catch(e){} const rec={name,business:data.business||"",city:(data.area||manualCity||"").split(/[,.]/)[0].replace(/[^a-zA-Z\s]/g,"").trim(),answers:data,post:gp!==undefined?gp:(prev.post||""),status:status||prev.status||"Post Generated",postGenerated:true,timestamp:prev.timestamp||Date.now(),updatedAt:Date.now()}; await window.storage.set(key,JSON.stringify(rec),true); } catch(e) {}
  }

  async function handleGeneratePost(ans) {
    const a=ans||answers; setPostLoading(true); setPostError("");
    try { const g=await generateAIPost(a); const c=g.replace(/\u2014/g,",").replace(/--/g,",").trim(); if(!c||c.length<50)throw new Error("empty"); setPost(c); setCompletedSections(p=>p.includes("write")?p:[...p,"write"]); await saveSubmission(a,c,"Post Generated"); }
  ...
  // Content width: full when no sidebar, offset when sidebar visible
  const contentStyle = {
  ...
  return (
    <div style={{ minHeight:"100vh", background:GRAY100, fontFamily:"'Inter',-apple-system,sans-serif" }}>
      ...
      <div style={{ background:NAVY, padding:"0 20px", height:HEADER_H, display:"flex", alignItems:"center", justifyContent:"space-between", position:"fixed", top:0, left:0, right:0, zIndex:160, boxShadow:"0 2px 12px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ display:"inline-flex", border:"1px solid rgba(255,255,255,0.25)", borderRadius:8, overflow:"hidden" }}>
            {["en","es"].map(l => (
              <button key={l} onClick={() => setLang(l)} style={{ background:lang===l?YELLOW:"transparent", color:lang===l?NAVY:WHITE, border:"none", padding:"5px 12px", fontWeight:700, fontSize:12, cursor:"pointer", transition:"all 0.15s" }}>
                {l==="en"?"EN":"ES"}
              </button>
            ))}
          </div>
      ...
      {/* Main content shifts right when sidebar is showing */}
      <div style={{ paddingTop:HEADER_H, ...contentStyle }}>
        <div ref={topRef} style={{ maxWidth:680, margin:"0 auto", padding:"28px 16px 60px" }}>
          ...
          {appPhase==="lane" && (
            <div>
              <div style={{ background:NAVY, borderRadius:16, padding:32, marginBottom:20, textAlign:"center" }}>
                ...
                <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>{["⏱ ~29 min total","🎯 10 group posts","💬 Real leads, real jobs"].map((s,i)=><div key={i} style={{ background:"rgba(255,255,255,0.08)", borderRadius:10, padding:"10px 18px", fontSize:13, color:WHITE }}>{s}</div>)}</div>
              </div>
              <button onClick={() => { if(answeredN===0)setAppPhase("writechoice"); else if(!post)setAppPhase("getpost"); else setAppPhase("checklist"); }} style={{ width:"100%", background:YELLOW, border:"none", borderRadius:14, padding:"18px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer", marginBottom:24, boxShadow:"0 4px 20px rgba(254,183,5,0.35)" }}>
                <div style={{ textAlign:"left" }}><div style={{ fontSize:11, fontWeight:700, color:NAVY, opacity:0.6, marginBottom:3, textTransform:"uppercase", letterSpacing:"0.05em" }}>Continue where you left off</div><div style={{ fontSize:16, fontWeight:900, color:NAVY }}>{answeredN===0?"Start Writing Your Post":!post?"Generate Your Post":"Cross-Post to More Groups"}</div></div>
              ...
              <div style={{ background:"#FEF9EC", border:"1.5px dashed "+YELLOW, borderRadius:10, padding:"10px 16px" }}>
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  {[{label:"Fill Sample Answers",fn:()=>{setAnswers(SAMPLE_ANSWERS);setAppPhase("ch1");}},{label:"Skip to Generate Post",fn:()=>{setAnswers(SAMPLE_ANSWERS);setAppPhase("getpost");}},{label:"Skip to Leads",fn:()=>{setAnswers(SAMPLE_ANSWERS);setPost("Sample post.");setAppPhase("leads");}}].map((b,i)=><button key={i} onClick={b.fn} style={{ background:NAVY, color:YELLOW, border:"none", borderRadius:8, padding:"6px 14px", fontSize:12, fontWeight:700, cursor:"pointer" }}>{b.label}</button>)}
                </div>
              </div>
            </div>
          )}

          {appPhase==="writechoice" && (<Card><SectionHeader emoji="✍️" title="Write Your Post" subtitle="How would you like to answer the questions?"/><div style={{ display:"flex", flexDirection:"column", gap:12, marginTop:8 }}><button onClick={()=>setAppPhase("ch1")} style={{ background:WHITE, border:"2px solid "+NAVY, borderRadius:14, padding:20, cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:16 }}><div style={{ fontSize:32, flexShrink:0 }}>⌨️</div><div><div style={{ fontWeight:800, color:NAVY, fontSize:15, marginBottom:4 }}>Type My Answers</div><div style={{ fontSize:13, color:GRAY600, lineHeight:1.5 }}>Fill in each question at your own pace. Mic icon available to dictate.</div></div></button><button onClick={()=>setAppPhase("voice")} style={{ background:NAVY, border:"2px solid "+NAVY, borderRadius:14, padding:20, cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:16 }}><div style={{ fontSize:32, flexShrink:0 }}>🎤</div><div><div style={{ fontWeight:800, color:YELLOW, fontSize:15, marginBottom:4 }}>Talk Through It</div><div style={{ fontSize:13, color:GRAY400, lineHeight:1.5 }}>Speak your answers. An AI coach guides you through every question.</div></div></button></div><CardNav onBack={()=>setAppPhase("lane")}/></Card>)}

          {appPhase==="voice" && <VoiceMode onComplete={va=>{setAnswers(va);setAppPhase("getpost");handleGeneratePost(va);}}/>}

          {(appPhase==="ch1"||appPhase==="ch2"||appPhase==="ch3") && <TypeMode onComplete={va=>{setAnswers(va);setAppPhase("groups");}} savedAnswers={answers} onAnswerChange={(id,val)=>setAnswers(prev=>({...prev,[id]:val}))}/>}

          {appPhase==="groups" && (<Card><SectionHeader emoji="🧭" title="Step 1 — Join a Group" subtitle="Find an active local Facebook group and join it. You only need one to start."/>{!answers.area&&(<div style={{ background:"#EFF6FF", borderRadius:10, padding:"12px 16px", marginBottom:16 }}><p style={{ margin:"0 0 8px", fontSize:13, fontWeight:600, color:NAVY }}>What city or area do you serve?</p><div style={{ display:"flex", gap:8 }}><input value={manualCity} onChange={e=>setManualCity(e.target.value)} placeholder="e.g. East Nashville" style={{ flex:1, border:"2px solid "+GRAY200, borderRadius:8, padding:"8px 12px", fontSize:14, outline:"none", fontFamily:"inherit" }}/><button onClick={()=>{setGroups5([]);setGroupsError("");setGroupsLoading(true);findFacebookGroups(manualCity||"Nashville",5).then(r=>{setGroups5(r);setGroupsLoading(false);}).catch(()=>setGroupsLoading(false));}} style={{ background:NAVY, color:YELLOW, border:"none", borderRadius:8, padding:"8px 16px", fontWeight:700, fontSize:13, cursor:"pointer" }}>Search</button></div></div>)}{groupsLoading&&<div style={{ textAlign:"center", padding:32 }}><p style={{ color:GRAY600 }}>Finding groups near {city}...</p></div>}{groupsError&&<div style={{ background:"#FEF2F2", borderRadius:10, padding:14, marginBottom:16, color:RED, fontSize:13 }}>{groupsError}</div>}              {!groupsLoading&&groups5.length>0&&(<><div style={{ background:"#EFF6FF", borderRadius:10, padding:"10px 14px", marginBottom:14, fontSize:13, color:NAVY, lineHeight:1.6 }}>💡 These are search suggestions — each button opens a Facebook search for that group type in your area. Browse the results and join a public group.</div><GroupTable groups={groups5} showNum={false}/></>)}<CardNav onBack={()=>setAppPhase("lane")} onNext={()=>setAppPhase("getpost")}/></Card>)}
          ...
          {appPhase==="dopost" && (<Card><SectionHeader emoji="🚀" title="Step 4 — Post It" subtitle="Follow these steps exactly. Use your personal account, not your business page."/><div style={{ background:"#FEF2F2", border:"1.5px solid "+RED, borderRadius:12, padding:"14px 18px", marginBottom:24, display:"flex", gap:12, alignItems:"flex-start" }}><span style={{ fontSize:20, flexShrink:0 }}>⚠️</span><div><p style={{ fontWeight:800, color:"#991B1B", fontSize:14, margin:"0 0 4px" }}>Post from your PERSONAL account, not your business page.</p><p style={{ fontSize:13, color:"#991B1B", margin:0, lineHeight:1.6 }}>Facebook shows personal posts to way more people. Log in as yourself before you post.</p></div></div>{[{num:1,text:"Open the Facebook group you joined and tap Write something"},{num:2,text:"Paste your copied post"},{num:3,text:"Attach your photo"},{num:4,text:"Tap Post"}].map(s=>(<div key={s.num} style={{ display:"flex", gap:14, marginBottom:18, alignItems:"flex-start" }}><div style={{ background:YELLOW, color:NAVY, borderRadius:99, width:36, height:36, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:16, flexShrink:0 }}>{s.num}</div><p style={{ margin:0, fontSize:15, color:GRAY800, paddingTop:7, lineHeight:1.6 }}>{s.text}</p></div>))}<CardNav onBack={()=>setAppPhase("photo")} onNext={()=>setAppPhase("approval")}/></Card>)}

          {appPhase==="approval" && (<Card><SectionHeader emoji="📋" title="Step 5 — Coach Approval"/><div style={{ background:"#EFF6FF", borderRadius:12, padding:24, marginBottom:24, textAlign:"center" }}><div style={{ fontSize:48, marginBottom:10 }}>👋</div><p style={{ color:NAVY, fontWeight:700, fontSize:15, margin:"0 0 8px" }}>Your post is live!</p><p style={{ color:GRAY600, fontSize:14, lineHeight:1.7, margin:0 }}>Let your coach know you are ready for your audit. Once reviewed and approved, tap Next.</p></div><CardNav onBack={()=>setAppPhase("dopost")} onNext={()=>{saveSubmission(answers,post,"Coach Approved");setAppPhase("joingroups");}}/></Card>)}

          {appPhase==="replicate" && (<Card><SectionHeader emoji="🔁" title="Step 6 — Join and Cross-Post" subtitle="Find 9 more public groups, join them, and post in each one. Same post. Same photo. No edits."/><div style={{ background:GRAY50, borderRadius:12, padding:16, marginBottom:24 }}>{["Search for local groups using the links below and join 9 more public groups.","Use the exact same post — do not change a single word.","Attach the exact same photo. Do not add your phone number, website, or any contact info.","Post immediately in each public group once you have joined."].map((s,i)=>(<div key={i} style={{ display:"flex", gap:10, marginBottom:8, alignItems:"flex-start" }}><div style={{ background:YELLOW, color:NAVY, borderRadius:99, width:22, height:22, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, fontSize:11, flexShrink:0, marginTop:1 }}>{i+1}</div><p style={{ margin:0, fontSize:13, color:GRAY800, lineHeight:1.6 }}>{s}</p></div>))}</div>{groupsLoading&&<p style={{ color:GRAY600, fontSize:14, textAlign:"center" }}>Loading group suggestions...</p>}{!groupsLoading&&groups20.length>0&&<div style={{ marginBottom:20 }}><GroupTable groups={groups20} showNum={false}/></div>}<CardNav onBack={()=>setAppPhase("approval")} onNext={()=>{setCompletedSections(p=>p.includes("grouppost")?p:[...p,"grouppost"]);saveSubmission(answers,post,"10 Groups Done");setAppPhase("celebrate");}} nextLabel="Next"/></Card>)}
