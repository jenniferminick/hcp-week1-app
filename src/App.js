...
const SAMPLE_ANSWERS = {
  ...
  fear: "Lack of communication. I send a quick text update any time something changes on a job so nobody is left wondering what is happening or when I will be done.",
  humanDetail: "Most Saturdays I am on the sidelines at my son's baseball game with a cooler and way too much sunscreen, cheering like it is the World Series.",
  localFlavor: "Mas Tacos Por Favor in East Nashville. I always get the chicken taco plate and eat it standing outside on the sidewalk no matter what the weather is doing.",
  mission: "hot chicken",
  whyStarted: "I remember sitting in my truck after a call feeling sick because my old company wanted me to push a bigger job the homeowner did not need. I drove home thinking I cannot build a life doing this.",
  whatChanged: "I am home to eat dinner with my family instead of coming in after everyone is asleep, and I coached my son's baseball team this spring for the first time.",
  heroMoment: "An elderly woman called me at 7pm on Christmas Eve. Her heat had been out for two days and her grandkids were coming the next morning. I drove 40 minutes, worked two hours in the cold, and only charged her for the part. She grabbed my hand on the way out and said she was going to tell everyone she knew about me. She has sent me four referrals since that night.",
};

// ── 10 canonical questions — verbatim, do not edit ────────────────────────────
const ALL_QUESTIONS = [
  {
  ...
  },
  {
    ...
    voiceQ: "What is your business name exactly as it appears online — the exact spelling someone would see on Google or Facebook?",
  ...
  },
  {
    ...
    voiceQ: "What is your service area — your main city plus any nearby towns — and how long have you been serving it?",
  ...
  },
  {
    ...
    voiceQ: "Homeowners have three big fears when hiring a contractor: getting overcharged, being left with a mess, or lack of communication. Which one do you focus on overcoming most, and what is one thing you do on every single job so customers never feel that fear?",
    placeholder: "e.g. Lack of communication. I send a quick update any time something changes so nobody is left wondering.",
  ...
  },
  {
    ...
    voiceQ: "Tell me one specific real life detail about you that neighbors would relate to. It could be a family moment, a hobby, a weekend routine, something you are working toward, or something you are just weirdly into.",
    placeholder: "e.g. Most Saturdays I'm on the sidelines at my kid's softball game with a cooler and way too much sunscreen.",
  },
  {
    ...
    voiceQ: "What is one specific local place you genuinely love? Give me the real name — a park, trail, restaurant, market, whatever it is — and tell me what you always do or get there.",
  ...
  },
  {
    ...
    voiceQ: "Fill in the blank for me: I am on a mission to find the best blank in my city. What is it? Just say the thing — tacos, pizza, coffee, whatever people have strong opinions on.",
  ...
  },
  {
    ...
    voiceQ: "After starting your own business, what is one real thing you can do now that you could not before? Make it something a neighbor could picture in one sentence.",
  ...
  },
  {
    ...
    voiceQ: "Tell me one Hero Moment story. A time you went above and beyond for a customer. What was happening, what did you do, and what did they say or do after? Include one detail you actually remember.",
    placeholder: "What was happening…\nWhat did you do…\nWhat did they say or do after…",
    heroFollowUps: [
      "What was happening when they called you?",
      "What did you actually do to help them?",
      "What did they say or do after you were done?",
      "What is one specific detail from that moment you still remember?",
    ],
  },
];

// ── Q4 / Q7 chip config pulled from question objects above ────────────────────
// (fearChips and missionChips live on the question objects directly)

// ── Per-question validation rules ─────────────────────────────────────────────
const FILLER_REGEX = /^(i mean|uh|um|like|okay|ok|yeah|yes|no|hold on|wait|sorry|hmm|hm|ah|oh|er|right|sure|well|so|and|the|a|an|just|okay so|alright|hi|hello|hey)[\s.,!?]*$/i;
...
function validateAnswer(q, text) {
  ...
  if (q.validate === "name") {
    const nameWords = t.replace(/[^a-zA-Z\s'-]/g, "").trim().split(/\s+/).filter(Boolean);
    if (nameWords.length < 2) return { ok: false, reason: "not_a_name" };
    const nonNamePhrases = /\b(timer|alarm|minute|second|hey|hello|siri|google|alexa|cancel|stop|okay)\b/i;
    if (nonNamePhrases.test(t)) return { ok: false, reason: "not_a_name" };
  }

  if (q.validate === "business") {
    if (words.length < 1) return { ok: false, reason: "short" };
  }

  if (q.validate === "area") {
    const hasDuration = /\b(\d+\s*(year|yr|month|decade)|for\s+\d+|going on|almost|over|nearly|new to|just moved|building our life)/i.test(t);
    if (!hasDuration) return { ok: false, reason: "missing_duration" };
  }

  if (q.validate === "localFlavor") {
    if (words.length < 6) return { ok: false, reason: "needs_more" };
  }

  if (q.validate === "hero") {
    if (words.length < 25) return { ok: false, reason: "short" };
  }

  return { ok: true };
}

// ── Strict per-question follow-up templates ───────────────────────────────────
const FOLLOWUP_TEMPLATES = {
  name:        ["I did not catch a full name there — what is your first and last name?"],
  business:    ["What is the exact name of your business as it appears on Google or Facebook?"],
  area:        {
    missing_duration: ["And how long have you been serving that area? Or if you are new, tell me one line about why you chose it."],
    default:          ["How long have you been serving that area?"],
  },
  fear:        ["Pick one of the three fears — overcharged, left with a mess, or lack of communication — and tell me one specific thing you do every job to prevent it."],
  humanDetail: ["Add one more detail — a name, a number, a place, or a year. What makes that part of your life real?"],
  localFlavor: ["Give me the actual name of the spot and one thing you always do or order there."],
  mission:     ["Just say the one thing — tacos, pizza, donuts, coffee, whatever people have strong opinions on around here."],
  whyStarted:  ["What were you actually feeling in that moment? Add that one detail."],
  whatChanged: ["Make it something a neighbor can picture — one real thing that is different in your daily life now."],
  heroMoment:  ["Add one more detail — what did they say, what did you see, or what do you still remember from that moment?"],
};
...
async function generateAIPost(ans) {
  ...
  const p = "You are a professional copywriter writing scroll-stopping neighborly Facebook posts for home service business owners.\n\n"
    + "STRICT NO-HALLUCINATION RULE: Use ONLY the facts explicitly stated in the answers below. Never invent names, places, years, prices, customer quotes, or outcomes. If something is not in the answers, leave it out.\n\n"
    + "Answers:\n"
    + "1. Name: " + (ans.name || "") + "\n"
    + "2. Business: " + (ans.business || "") + "\n"
    + "3. Area: " + (ans.area || "") + "\n"
    + "4. Customer fear they overcome: " + (ans.fear || "") + "\n"
    + "5. Real life detail: " + (ans.humanDetail || "") + "\n"
    + "6. Local flavor: " + (ans.localFlavor || "") + "\n"
    + "7. Mission fill-in: " + (ans.mission || "") + "\n"
    + "8. Why they started: " + (ans.whyStarted || "") + "\n"
    + "9. What changed after starting: " + (ans.whatChanged || "") + "\n"
    + "10. Hero moment story: " + (ans.heroMoment || "") + "\n\n"
    + "RULES: First person only (I, me, my). Never use: professionalism, integrity, excellence, quality work, commitment, reliable, expert, top-notch. No em dashes. No CTA, phone number, or website. Fix all grammar. 330-450 words. Short paragraphs.\n"
    + "STRUCTURE:\n"
    + "(1) Vulnerable hook — open with the real moment from why they started or the hero moment crisis\n"
    + "(2) Identity anchor — name, business, area, how long they have been there\n"
    + "(3) The fear they overcome and the specific habit they use\n"
    + "(4) Hero moment told as a vivid short scene using only the facts provided\n"
    + "(5) Neighbor proof — real life detail and local flavor woven in naturally\n"
    + "(6) End ONLY with: Also, I'm on a mission to find the best [mission] in [city from area]. Any suggestions?\n\n"
    + "Output the post only. No labels, no headings, no bullet points.";
...
function VoiceMode({ onComplete, lang }) {
  ...
  function coachSay(msg, afterSpeak) {
    ...
    ttsSpeak(msg, ttsLang, ()=>{ S.current.busy=false; if (afterSpeak) afterSpeak(); else openMic(); }, currentAudioRef);
  }
  ...
  function resumeSession() {
    ...
    const msg = lang==="es" ? "Bienvenido de vuelta. Continuemos. "+vq : "Welcome back! Let us pick up where we left off. "+vq;
  ...
  async function handleAnswer(text) {
    ...
    // ── Step 1: Hard validation BEFORE touching Claude ──────────────────────
    const validation = validateAnswer(q, text);

    if (!validation.ok) {
      // Determine the exact retry prompt — no Claude call needed for invalid answers
      let retryMsg = "";
      const templates = FOLLOWUP_TEMPLATES[q.id];

      if (q.id === "area" && validation.reason === "missing_duration") {
        retryMsg = templates.missing_duration[0];
      } else if (q.id === "area" && validation.reason === "missing_city") {
        retryMsg = templates.missing_city[0];
      } else if (validation.reason === "not_a_name" || validation.reason === "filler") {
        retryMsg = Array.isArray(templates) ? templates[0] : templates.default[0];
      } else if (validation.reason === "generic_words") {
        retryMsg = "Avoid words like reliable or honest — give me the specific 2-3 word phrase your best customers would use to describe you.";
      } else {
        retryMsg = Array.isArray(templates) ? templates[0] : templates.default ? templates.default[0] : q.voiceQ;
      }

      // Track follow-up count — max 1 per question
      S.current.followupCount = (S.current.followupCount || 0) + 1;
      if (S.current.followupCount >= 2) {
        // Accept whatever they last said after 1 follow-up to avoid infinite loop
        // (but only if it's not pure filler)
        if (text.trim().length > 3) {
          // fall through to accept logic below
        } else {
          S.current.busy = false; openMic(); return;
        }
      } else {
        setTranscript("");
        coachSay(retryMsg, () => openMic());
        return;
      }
    }

    // Reset follow-up counter for the next question
    S.current.followupCount = 0;

    // ── Step 2: Answer is valid — generate warm acknowledgment via Claude ───
    const nextVoiceQ = nextQ ? (nextQ.voiceQ || nextQ.hint) : null;

    const sysPrompt = "You are a warm conversational business coach. STRICT RULES:\n"
      + "1. NEVER use filler words: Perfect, Love that, Great, Awesome, Got it, Amazing, Wonderful, Fantastic.\n"
      + "2. React to ONE specific detail from their answer — be genuinely surprised, curious, or moved.\n"
      + "3. Keep your response to 1-2 sentences maximum.\n"
      + "4. After your reaction, ask EXACTLY this next question and nothing else: " + (nextVoiceQ || "[end]") + "\n"
      + "5. NEVER ask about topics not in the next question. NEVER introduce new subjects.\n"
      + "6. Output format: Start with ACCEPT then your message. Nothing else.";

    const userPrompt = "They answered Q" + q.num + " (" + q.label + ").\n"
      + "Their answer: \"" + text + "\"\n"
      + (isLast
        ? "This is the last question. Start with ACCEPT then say: " + t.voiceDone
        : "Start with ACCEPT, react briefly to something specific they said, then ask: " + nextVoiceQ);
    ...
    try {
      ...
      reply = "ACCEPT " + (nextVoiceQ || t.voiceDone);
    }
    ...
    // ── Step 3: Commit answer and advance ───────────────────────────────────
    const id = rerecordId || q.id;

    // For localFlavor, also split into localPlace + localActivity for post generation
    const newAnswers = { ...S.current.answers, [id]: text };
    if (id === "localFlavor") {
      // Store combined; post generator will use localFlavor directly
      newAnswers.localPlace = text;
      newAnswers.localActivity = text;
    }

    S.current.answers = newAnswers;
  ...
  useEffect(()=>{
    ...
    const r = new SR();
    r.continuous=false; r.interimResults=true; r.lang=ttsLang;
    ...
    r.onresult = e => {
      ...
      S.current.busy=true; S.current.wantMic=false;
    ...
    r.onend = () => {
      if (S.current.wantMic&&!S.current.busy) {
        setTimeout(()=>{ if (S.current.wantMic&&!S.current.busy) try { r.start(); } catch(_) {} },400);
  ...
  return (
    <div>
      ...
      {!paused&&uiStatus!=="idle"&&<div style={{ marginBottom:12 }}><ProgressBar current={answeredCount} total={ALL_QUESTIONS.length}/></div>}
      {uiStatus==="idle"&&!paused&&(
        <div style={{ textAlign:"center", marginTop:8, display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
          ...
          <button onClick={()=>{ S.current.answers={...SAMPLE_ANSWERS}; S.current.qIdx=ALL_QUESTIONS.length-1; setDisplayAnswers({...SAMPLE_ANSWERS}); setDisplayQIdx(ALL_QUESTIONS.length-1); setCoachMsg("Dev mode - all answers filled. Hit Continue."); setUiStatus("done"); }} style={{ background:"#1a1a2e", color:YELLOW, border:"1.5px dashed "+YELLOW, borderRadius:8, padding:"6px 16px", fontSize:12, fontWeight:700, cursor:"pointer" }}>Dev: Auto-fill all answers</button>
        </div>
      )}
      {answeredCount>0&&!paused&&(
        <Card>
...
function TypeMode({ onComplete, lang, savedAnswers, onAnswerChange }) {
  ...
  const handleInspiration = async () => {
    ...
    try {
      const reply = await callClaude([{ role: "user", content: "Give 3 short vivid example answers for this question from a home service business owner:\n\nQuestion: " + q.question + "\nContext: " + q.hint + "\n\nReturn ONLY a JSON array of 3 strings. Each must be clearly different. Authentic and specific. No markdown, no code fences." }]);
      const cleaned = reply.replace(/```json/gi,"").replace(/```/g,"").trim();
      const start = cleaned.indexOf("["); const end = cleaned.lastIndexOf("]");
      if (start !== -1 && end !== -1) { const p = JSON.parse(cleaned.slice(start, end+1)); setInspirationExamples(Array.isArray(p) ? p : []); }
    } catch(e) { setInspirationExamples(q.examples || []); }
    ...
    <div>
      ...
      <Card>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <span style={{ fontWeight: 800, color: NAVY, fontSize: 17, lineHeight: 1.4 }}>{q.question}</span>
        </div>
        ...
        {q.fearChips && (
          <div style={{ marginBottom: 14 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: GRAY400, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.05em" }}>Which fear do you focus on?</p>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {q.fearChips.map((chip, i) => {
                const selected = value.toLowerCase().includes(chip.toLowerCase());
                return (
                  <button key={i} onClick={() => {
                    const current = savedAnswers[q.id] || "";
                    if (!current.toLowerCase().includes(chip.toLowerCase())) {
                      onAnswerChange(q.id, chip + ". " + current);
                    }
                  }}
                    style={{ background: selected ? NAVY : WHITE, color: selected ? YELLOW : NAVY, border: "2px solid " + NAVY, borderRadius: 99, padding: "6px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "all 0.15s" }}>
                    {chip}
                  </button>
                );
              })}
            </div>
            <p style={{ fontSize: 11, color: GRAY400, margin: "8px 0 0" }}>Tap to add the fear you focus on, then write your full answer below.</p>
          </div>
        )}
        ...
        {q.missionChips && (
          <div style={{ marginBottom: 14 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: GRAY400, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.05em" }}>Quick pick</p>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {q.missionChips.map((chip, i) => (
                <button key={i} onClick={() => onAnswerChange(q.id, chip)}
                  style={{ background: value === chip ? NAVY : WHITE, color: value === chip ? YELLOW : NAVY, border: "2px solid " + NAVY, borderRadius: 99, padding: "6px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "all 0.15s" }}>
                  {chip}
                </button>
              ))}
            </div>
        ...
        <div style={{ position: "relative" }}>
          <textarea
            value={value}
            onChange={e => { onAnswerChange(q.id, e.target.value); if (showError) setShowError(false); }}
            placeholder={q.placeholder}
            rows={q.id === "heroMoment" ? 6 : q.id === "whyStarted" || q.id === "fear" ? 4 : 3}
            autoFocus
            style={{ width: "100%", boxSizing: "border-box", border: "2px solid " + (showError ? RED : value ? (met ? GREEN : GRAY300) : GRAY200), borderRadius: 12, padding: "14px 52px 14px 14px", fontSize: 15, color: GRAY800, fontFamily: "inherit", resize: "vertical", outline: "none", transition: "border 0.15s", background: value ? (met ? "#F0FDF4" : WHITE) : WHITE, lineHeight: 1.7 }}
          />
        ...
        {/* Word count bar — only shown after they start typing */}
        {wc > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "8px 0 4px" }}>
            <div style={{ flex: 1, background: GRAY200, borderRadius: 99, height: 5, overflow: "hidden" }}>
              <div style={{ background: met ? GREEN : GRAY400, height: 5, borderRadius: 99, width: Math.min((wc / q.minWords) * 100, 100) + "%", transition: "width 0.2s" }} />
            </div>
            <span style={{ fontSize: 12, fontWeight: 700, color: met ? GREEN : GRAY400, whiteSpace: "nowrap" }}>{met ? "✓ " : ""}{wc} / {q.minWords} words min</span>
          </div>
        )}
        {wc === 0 && (
          <p style={{ fontSize: 12, color: GRAY400, margin: "6px 0 0" }}>Minimum {q.minWords} word{q.minWords !== 1 ? "s" : ""} required</p>
        )}

        {/* Error shown only on attempted continue */}
        {showError && !met && (
          <div style={{ background: "#FEF2F2", border: "1.5px solid #FCA5A5", borderRadius: 10, padding: "10px 14px", marginTop: 10, fontSize: 13, color: "#991B1B", fontWeight: 600 }}>
            Add one more detail like a name, place, number, or year.
          </div>
        )}

        {/* Need inspiration */}
        <div style={{ marginTop: 14 }}>
          <button onClick={handleInspiration} style={{ background: "transparent", border: "2px dashed " + GRAY300, borderRadius: 10, padding: "9px 18px", fontSize: 13, color: GRAY600, cursor: "pointer", fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6 }}>
            💡 Need inspiration?
          </button>
        </div>
        ...
        {showInspiration && (
          <div style={{ marginTop: 10, background: "#FFFBEB", border: "1.5px solid " + YELLOW, borderRadius: 12, padding: 16 }}>
            ...
            <p style={{ fontSize: 11, color: GRAY400, margin: "10px 0 0" }}>These are guides only — write your own real version.</p>
          </div>
        )}
      </Card>

      <BottomNav onBack={qIdx > 0 ? handleBack : undefined} onNext={handleNext} nextDisabled={false} nextLabel={isLast ? "Generate My Post →" : "Next →"} />
      <NavSpacer />
    </div>
  );
}
      ...
      {!active&&(
        <Card>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            {LEAD_TYPES.map(lt=>(
              <button key={lt.id} onClick={()=>{ setActive(lt.id); setSubtype(null); }}
...
export default function App() {
  ...
  useEffect(()=>{
    try {
      ...
      if (raw) { const saved=JSON.parse(raw); if (saved&&saved.answers&&Object.keys(saved.answers).length>0) { setAnswers(saved.answers); if (saved.post) setPost(saved.post); if (typeof saved.postCount==="number") setPostCount(saved.postCount); setDraftRestored(true); setTimeout(()=>setDraftRestored(false),4000); } }
    } catch(e) {}
  },[]);
      ...
      <div ref={topRef} style={{ maxWidth:960, margin:"0 auto", padding:"28px 32px 60px" }}>
        ...
        {appPhase==="lane"&&(
          <div>
            ...
            <button onClick={()=>{ if (answeredN===0) setAppPhase("writechoice"); else if (!post) setAppPhase("getpost"); else if (postCount<9) setAppPhase("replicate"); else setAppPhase("leads"); }}
              ...
              <div style={{ textAlign:"left" }}>
                ...
                <div style={{ fontSize:16, fontWeight:900, color:NAVY }}>{answeredN===0?ht.cta0:!post?ht.cta1:postCount<9?ht.cta2:ht.cta3}</div>
              </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:24 }}>
              {[
                { phase:"writechoice", icon:"✍️", bg:NAVY, title:ht.writeTitle, desc:ht.writeDesc, time:ht.writeTime, progress:answeredN+" "+ht.ofAnswered, status:answeredN===0?{label:ht.statusNotStarted,bg:GRAY100,fg:GRAY400}:answeredN<15?{label:ht.statusInProgress,bg:"#FEF9EC",fg:"#92400E"}:{label:ht.statusDone,bg:"#D1FAE5",fg:"#065F46"} },
                ...
                <div key={item.phase} onClick={()=>setAppPhase(item.phase)} style={{ background:WHITE, borderRadius:16, boxShadow:"0 2px 12px rgba(0,41,66,0.06)", overflow:"hidden", cursor:"pointer", display:"flex" }}>
                  ...
                  <div style={{ padding:"14px 16px", flex:1 }}>
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
                      ...
                      <span style={{ background:item.status.bg, color:item.status.fg, borderRadius:99, padding:"3px 10px", fontSize:11, fontWeight:700 }}>{item.status.label}</span>
                    </div>
        ...
        {(appPhase==="ch1"||appPhase==="ch2"||appPhase==="ch3")&&(
          <TypeMode onComplete={va=>{ setAnswers(va); setAppPhase("groups"); }} lang={lang} savedAnswers={answers} onAnswerChange={(id,val)=>setAnswers(prev=>({...prev,[id]:val}))}/>
        )}
        ...
        {appPhase==="getpost"&&(
          <GetPost allCh3Met={allCh3Met} post={post} postLoading={postLoading} postError={postError} answers={answers} onGenerate={handleGeneratePost} onSetPost={setPost} onNext={()=>setAppPhase("copypost")} onBack={()=>setAppPhase("groups")} onWritePost={()=>setAppPhase("writechoice")}/>
        )}
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(155px, 1fr))", gap:10, marginBottom:20 }}>
                {[{emoji:"😊",label:"You with family",desc:"Smiling face, bright natural light."},{emoji:"☕",label:"You at your local spot",desc:"Candid shot where you hang out."},{emoji:"🔧",label:"Candid job-site photo",desc:"You working, face clearly visible."}].map((g,i)=>(
                  <div key={i} style={{ background:"#F0FDF4", border:"1.5px solid #86EFAC", borderRadius:12, padding:14, textAlign:"center" }}>
                    ...
                    <div style={{ fontWeight:700, color:"#166534", fontSize:13, marginBottom:4 }}>{g.label}</div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(155px, 1fr))", gap:10 }}>
                {[{emoji:"🚛",label:"Truck only",desc:"No face = no connection."},{emoji:"🖼️",label:"Logo or flyer",desc:"Looks like an ad."},{emoji:"🕶️",label:"Dark selfie",desc:"Hard to connect with."},{emoji:"🤖",label:"AI or stock photo",desc:"People can tell."}].map((b,i)=>(
                  <div key={i} style={{ background:"#FEF2F2", border:"1.5px solid #FCA5A5", borderRadius:12, padding:14, textAlign:"center" }}>
                    ...
                    <div style={{ fontWeight:700, color:"#991B1B", fontSize:13, marginBottom:4 }}>{b.label}</div>
        ...
        {appPhase==="replicate"&&(
          <>
            ...
            <Card>
              ...
              {postCount>0&&postCount<9&&<div style={{ background:"#FEF9EC", border:"1.5px solid "+YELLOW, borderRadius:10, padding:"12px 16px", marginBottom:16, fontSize:13, color:GRAY800, lineHeight:1.6 }}>Posted in <strong>{postCount+1} total groups</strong>. <strong>{9-postCount} more to go.</strong></div>}
