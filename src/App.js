...
const ALL_QUESTIONS = [
  ...
  { id:"knownFor",     num:4,    chapter:"ch1", minWords:3,  label:"Your Known For",              voiceQ:"If someone was describing you to a neighbor, what would they say you're the go-to person for?",                  hint:"If a neighbor was recommending you, what 2-3 specific words would they use?",                                    placeholder:"e.g. slow drain detective" },
  ...
  { id:"refuses",      num:"5b", chapter:"ch1", minWords:15, label:"What You Refuse to Do",       voiceQ:"What's one thing you flat-out refuse to do that others in your industry do?",                                    hint:"What's one thing you flat-out refuse to do that others in your industry do? Be specific.",                      placeholder:"e.g. I will never tell someone they need a full repipe just to run up a bill" },
  { id:"humanDetail",  num:6,    chapter:"ch2", minWords:15, label:"One Human Detail",            voiceQ:"Tell me something real about you outside of work with specifics.",                                               hint:"What's something real about you outside of work? Give specifics — year, color, name.",                          placeholder:"e.g. I have been restoring a 1967 candy apple red Ford Bronco" },
  { id:"localPlace",   num:7,    chapter:"ch2", minWords:4,  label:"Local Flavor - Place",        voiceQ:"What's one specific local spot you genuinely love? Give me the actual name.",                                    hint:"What's one specific local spot you genuinely love? Give the actual name.",                                      placeholder:"e.g. Mas Tacos Por Favor in East Nashville" },
  { id:"localActivity",num:"7b", chapter:"ch2", minWords:10, label:"What You Do There",           voiceQ:"What do you always do or order when you're there?",                                                              hint:"What do you always do or order when you're there? Paint the picture.",                                          placeholder:"e.g. always get the chicken taco plate and eat it standing outside" },
  ...
  { id:"heroCrisis",   num:10,   chapter:"ch3", minWords:20, label:"Hero Moment - The Crisis",    voiceQ:"Tell me about a time you went above and beyond for a customer. Who was in trouble, what was at stake?",         hint:"Tell me about a time you went above and beyond. Who was in trouble, what was at stake?",                        placeholder:"e.g. An elderly woman called me at 7pm on Christmas Eve" },
  ...
  { id:"heroPayoff",   num:"10c",chapter:"ch3", minWords:10, label:"Hero Moment - The Payoff",    voiceQ:"What was the reaction from them or from you?",                                                                   hint:"What was the reaction — from them or from you? What did that moment feel like?",                               placeholder:"e.g. She grabbed my hand on the way out" },
];
...
const LEAD_TYPES = [
  ...
  { id:"dm", emoji:"✉️", label:"Direct Message", color:"#FDF4FF", border:"#D8B4FE", simple:false,
    subtypes:[
      ...
      { id:"ds2",  label:"Asked about a service you do not offer", example:'"Do you do roof repairs?"',                        dmScript:"Hey, thanks so much for reaching out. I really appreciate it. I was not sure how that post would land, so it means a lot that it resonated. We do not provide [service] but we do provide [your services]. Is there another item on your to-do list we can help with?" },
      { id:"ds3",  label:"General question or area check",         example:'"Do you service Hermitage?"',                      dmScript:"Hey, thanks so much for reaching out. I really appreciate it. I was not sure how that post would land, so it means a lot that it resonated. [Insert your answer to their question here.]" },
      { id:"ds4",  label:"Needs help now",                         example:'"I was just about to call someone!"',              dmScript:"Hey, thanks so much for reaching out. I really appreciate it. I was not sure how that post would land, so it means a lot that it resonated. "+BOOKING },
      ...
      { id:"ds7",  label:"Referral or future interest",            example:'"Can you help my friend?"',                        dmScript:"Hey, thanks so much for reaching out. I really appreciate it. The best compliment someone can give us is to refer us to someone else. We would love to help them out. Could you share their name and phone number?" },
...
const T = {
  en: { voiceIntro:"Hey! I'm going to walk you through a series of questions to build your story. Tap Start and I'll guide you through everything.", voiceStart:"Start Voice Session", voiceDone:"That's everything I need. Amazing work. Let's build your post now.", voiceContinue:"Continue to Post in Groups →", listenHint:"Listening — speak your answer", processing:"Processing...", answeredSoFar:"Your Answers So Far", rerecord:"🎤 Re-record", seeQuestion:"See current question" },
  es: { voiceIntro:"¡Hola! Voy a guiarte por una serie de preguntas para construir tu historia.", voiceStart:"Iniciar Sesión de Voz", voiceDone:"Eso es todo. Excelente trabajo.", voiceContinue:"Continuar →", listenHint:"Escuchando — di tu respuesta", processing:"Procesando...", answeredSoFar:"Tus Respuestas Hasta Ahora", rerecord:"🎤 Re-grabar", seeQuestion:"Ver pregunta actual" },
};
  ...
  return (
    <div style={{ marginBottom:8 }}>
      <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:GRAY400, marginBottom:4 }}>
        <span>{current} of {total} answered</span><span>{pct}%</span>
      </div>
      <div style={{ background:GRAY200, borderRadius:99, height:6 }}>
        <div style={{ background:YELLOW, borderRadius:99, height:6, width:pct+"%", transition:"width 0.4s" }} />
      </div>
    </div>
  );
}
...
function MicBtn({ onTranscript, size, lang }) {
  ...
  useEffect(() => {
    ...
    if (!SR) return;
    const r = new SR();
    r.continuous = false; r.interimResults = false; r.lang = lang === "es" ? "es-ES" : "en-US";
      ...
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
        <thead>
          <tr style={{ background:NAVY }}>
            {(showNum ? ["#"] : []).concat(["Group Name","Privacy","Members","Type"]).map(h => (
              <th key={h} style={{ padding:"10px 12px", color:YELLOW, textAlign:"left", fontWeight:700 }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
