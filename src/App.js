...
const HARPER_VOICE_ID = "QLAlOeRuLwKX0skeTR7R";
...
async function speakEL(text) {
  ...
  try {
    const res = await fetch("/api/tts", {
      ...
      body: JSON.stringify({ text, voiceId: HARPER_VOICE_ID }),
    });
    if (!res.ok) return;
...
function VoiceMode({ onComplete }) {
  ...
  function handleStart() {
    setStarted(true);
    const intro = "Hey, I'm going to walk you through 15 questions to build your story. Take your time — I'll let you know if I need more detail. Let's start: " + ALL_QUESTIONS[0].hint;
    setTimeout(() => coachSay(intro), 200);
  }
    ...
    <div>
      ...
      {started && !done && (
        <Card style={{ textAlign:"center" }}>
          ...
          {turn==="coach" && <div style={{ color:GRAY400, fontSize:13, padding:"8px 0" }}>Harper is speaking...</div>}
      ...
      {!started && <div style={{ textAlign:"center", marginTop:8 }}><Btn onClick={handleStart}>🎤 Start Voice Session</Btn><p style={{ fontSize:12, color:GRAY400, marginTop:8 }}>Make sure your volume is on — Harper will speak to you.</p></div>}
...
const SHIMMER_CSS = `@keyframes shimmer{0%{transform:translateX(-100%)}100%{transform:translateX(250%)}}@keyframes flashGreen{0%,100%{background:#10B981}50%{background:#34D399}}`;
const CHAPTER_META = {
  ...
  return (
    <div style={{ marginBottom:20 }}>
      ...
      <div style={{ background:GRAY200, borderRadius:99, height:10, overflow:"hidden", position:"relative" }}>
        <div style={{ height:10, borderRadius:99, width:visualPct+"%", background:complete?"#10B981":YELLOW, transition:"width 0.55s cubic-bezier(0.4,0,0.2,1), background 0.3s ease", position:"relative", overflow:"hidden" }}>
...
function LeadEngagement({ onBack, city, week1Total, onAmplify }) {
  ...
  return (
    <div>
      ...
      {!watched && (
        <Card>
          <SectionHeader emoji="🎉" title="Session Complete — Now We Execute" subtitle="Before working your leads, watch this quick training video." />
            ...
            <Card>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                <button onClick={() => { setLane("voice"); setAppPhase("voice"); }} style={{ background:NAVY, border:"2px solid "+NAVY, borderRadius:14, padding:20, cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:16 }}>
                  ...
                  <div><div style={{ fontWeight:800, color:YELLOW, fontSize:15, marginBottom:4 }}>Talk Through It</div><div style={{ fontSize:13, color:GRAY400, lineHeight:1.5 }}>Speak your answers. Harper guides you through every question.</div></div>
                </button>
              </div>
            </Card>
          </>
          ...
          <Card>
            ...
            {!answers.area && (
              <div style={{ background:"#EFF6FF", borderRadius:10, padding:"12px 16px", marginBottom:16 }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <input value={manualCity} onChange={e => setManualCity(e.target.value)} onKeyDown={e => { if (e.key==="Enter") { const c=manualCity.trim()||"Your City"; setGroups5([]); setGroupsError(""); setGroupsLoading(true); findFacebookGroups(c,5).then(r => { setGroups5(r); setGroupsLoading(false); if (!r.length) setGroupsError("No groups found."); }).catch(() => { setGroupsLoading(false); setGroupsError("Search failed."); }); } }} placeholder="e.g. East Nashville" style={{ flex:1, border:"2px solid "+GRAY200, borderRadius:8, padding:"8px 12px", fontSize:14, outline:"none", fontFamily:"inherit" }} />
                  <button onClick={() => { const c=manualCity.trim()||"Your City"; setGroups5([]); setGroupsError(""); setGroupsLoading(true); findFacebookGroups(c,5).then(r => { setGroups5(r); setGroupsLoading(false); if (!r.length) setGroupsError("No groups found."); }).catch(() => { setGroupsLoading(false); setGroupsError("Search failed."); }); }} style={{ background:NAVY, color:YELLOW, border:"none", borderRadius:8, padding:"8px 16px", fontWeight:700, fontSize:13, cursor:"pointer" }}>Search</button>
                </div>
              </div>
            )}
              ...
              {!postLoading && ch3Met && post && (
                <>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    ...
                    {!copiedGetpost && <div style={{ borderTop:"1px solid "+GRAY200, paddingTop:14, marginTop:4 }}><Btn variant="secondary" onClick={() => setAppPhase("photo")} style={{ fontSize:13 }}>Skip to Photo Step →</Btn></div>}
                  </div>
                </>
              )}
              {!postLoading && !ch3Met && (
