...
function speakCoach(text, onDone) {
  ...
  const voice = getCoachVoice(); if (voice) u.voice = voice;
...
function FbButton({ url }) {
  return (
    <a href={url} target="_blank" rel="noopener noreferrer"
      style={{ background:"#1877F2", color:WHITE, borderRadius:8, padding:"8px 14px", fontSize:13, fontWeight:700, textDecoration:"none", display:"inline-flex", alignItems:"center", gap:6, whiteSpace:"nowrap", flexShrink:0 }}>
...
function EvidenceCapture({ onSubmit }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:20, padding:16, borderRadius:12, border:"2px solid "+(urlValid?GREEN:GRAY200), background:urlValid?"#F0FDF4":WHITE, transition:"all 0.2s" }}>
        ...
        <input type="url" value={url} onChange={e => { setUrl(e.target.value); if (screenshot) { setScreenshot(null); setScreenshotName(""); } }} placeholder="https://www.facebook.com/groups/..."
          style={{ width:"100%", boxSizing:"border-box", border:"2px solid "+(urlValid?GREEN:GRAY200), borderRadius:10, padding:"10px 14px", fontSize:14, outline:"none", fontFamily:"inherit", background:"transparent", transition:"border 0.15s" }} />
        {urlValid && <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:8 }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={GREEN} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg><span style={{ fontSize:13, color:GREEN, fontWeight:700 }}>URL accepted — you're good to submit!</span></div>}
      </div>
      ...
      <div style={{ marginBottom:24, padding:16, borderRadius:12, border:"2px solid "+(screenshot?GREEN:GRAY200), background:screenshot?"#F0FDF4":WHITE, transition:"all 0.2s" }}>
        ...
        {!screenshot ? (
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div style={{ flex:1 }}><div style={{ fontWeight:700, color:"#065F46", fontSize:14 }}>{screenshotName}</div><div style={{ fontSize:12, color:GREEN, fontWeight:600 }}>✓ Screenshot accepted — you're good to submit!</div></div>
            <button onClick={() => { setScreenshot(null); setScreenshotName(""); }} style={{ background:"none", border:"none", color:GRAY400, fontSize:20, cursor:"pointer", padding:"4px 8px" }}>×</button>
          </div>
        )}
      </div>
...
function CoachDashboard({ onClose }) {
  ...
  async function doTestSave() {
    setTestMsg("Saving...");
    try { await window.storage.set("submission:test_pro", JSON.stringify({ name:"Test Pro", business:"Test Biz", city:"Nashville", answers:{}, post:"Test post", status:"Needs Review", evidenceUrl:"https://facebook.com/test", postGenerated:true, timestamp:Date.now(), updatedAt:Date.now() }), true); setTestMsg("Saved! Click Refresh."); setTimeout(loadData,600); }
      ...
      <div style={{ maxWidth:800, margin:"0 auto", padding:"28px 16px 60px" }}>
        ...
        {!loading && submissions.length === 0 && (
          <Card><div style={{ textAlign:"center", padding:20 }}>
            ...
            {testMsg && <p style={{ marginTop:14, fontWeight:700, fontSize:13, color:testMsg.startsWith("Error")?RED:GREEN }}>{testMsg}</p>}
          </div></Card>
        )}
          ...
          <Card key={i}>
            ...
            {expanded===i && (
              <div style={{ marginTop:16 }}>
                {(s.status==="Needs Review"||s.evidenceUrl) && (
                  <div style={{ background:"#FEF9C3", border:"1.5px solid #FCD34D", borderRadius:10, padding:"12px 16px", marginBottom:16 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:"#92400E", marginBottom:6 }}>⚑ SUBMITTED FOR REVIEW</div>
                    {s.evidenceUrl
                      ? <div><div style={{ fontSize:12, color:"#92400E", marginBottom:4 }}>Post URL:</div><a href={s.evidenceUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize:13, color:"#1D4ED8", wordBreak:"break-all" }}>{s.evidenceUrl}</a></div>
                      : <div style={{ fontSize:13, color:"#92400E" }}>Screenshot uploaded (view not available in dashboard)</div>}
                  </div>
                )}
...
function VoiceMode({ onComplete }) {
  ...
  function coachSay(text, afterSpeaking) {
    ...
    speakCoach(text, () => { if (!mountedRef.current) return; if (afterSpeaking) afterSpeaking(); else openMic(); });
  }
  ...
  async function handleUserSpeech(text) {
    ...
    if (isAccept) {
      ...
      if (!rerecordRef.current && isLast) { processingRef.current = false; setDone(true); coachSay(msg||"That's everything I need. You did great. Let's build your post.", () => {}); return; }
  ...
  function handleStart() {
    setStarted(true);
    setTimeout(() => { coachSay("Hey, I'm going to walk you through 15 questions to build your story. Take your time — I'll let you know if I need more detail. Let's start: "+ALL_QUESTIONS[0].hint); }, 200);
  }
  function handleRerecord(id) { setRerecordId(id); const q = ALL_QUESTIONS.find(x => x.id===id); coachSay("Sure, let's redo that one. "+q.hint); }
  function handleUserDone() { stopMic(); setTimeout(() => { const t = transcriptRef.current.trim(); if (t) handleUserSpeech(t); else coachSay("I didn't quite catch that. Could you say it again?", openMic); }, 150); }
  ...
  return (
    <div>
          ...
          <div style={{ flex:1 }}>
            {turn==="thinking" ? (
              <div style={{ display:"flex", gap:6, alignItems:"center", paddingTop:10 }}>
                {[0,1,2].map(i => <div key={i} style={{ width:8, height:8, borderRadius:"50%", background:YELLOW, opacity:0.6, animation:`pulse 1.2s ease-in-out ${i*0.2}s infinite` }} />)}
        ...
        <Card style={{ textAlign:"center" }}>
          ...
          {currentQ && (
            <details style={{ marginTop:12, textAlign:"left" }}>
              <summary style={{ fontSize:12, color:GRAY400, cursor:"pointer", userSelect:"none" }}>{chapterLabel} · Q{currentQ.num} of {ALL_QUESTIONS.length} · <span style={{ textDecoration:"underline" }}>See question text</span></summary>
              <div style={{ marginTop:8, padding:"10px 14px", background:GRAY50, borderRadius:10, fontSize:13, color:GRAY600, lineHeight:1.6 }}><strong style={{ color:NAVY }}>{currentQ.label}</strong><br />{currentQ.hint}</div>
            </details>
          )}
        </Card>
      )}
      {!started && <div style={{ textAlign:"center", marginTop:8 }}><Btn onClick={handleStart}>🎤 Start Voice Session</Btn><p style={{ fontSize:12, color:GRAY400, marginTop:8 }}>Make sure your volume is on — the coach will speak to you.</p></div>}
      ...
      {done && <div style={{ marginTop:8 }}><Btn onClick={() => onComplete(voiceAnswers)}>Continue to Post in Groups →</Btn></div>}
    </div>
  );
}
...
function Amplify({ city: initialCity, week1Total }) {
  ...
  async function fetchGroups(cityOverride) {
    ...
    if (!c || c === "Your City") { setError("Please enter your city above to find groups."); return; }
    setLoading(true); setError("");
    try {
      const fresh = await findFacebookGroups(c, 20);
      let result = fresh;
      if (fresh.length < 5 && prevGroups.length > 0) { const combined = [...fresh]; for (let g of prevGroups) { if (!combined.find(x => x.name===g.name)) combined.push(g); if (combined.length>=20) break; } result = combined; }
      setPrevGroups(groups); setGroups(result);
      if (!result.length) setError("No groups found for that city. Try a nearby city or different spelling.");
    } catch(e) { setError("Could not load groups. Check your connection and try again."); }
    setLoading(false);
  }

  useEffect(() => {
    if (activeCity && activeCity !== "Your City") fetchGroups(activeCity);
  }, []);
    ...
    <>
      <Card style={{ background:NAVY }}>
        ...
        <div style={{ marginTop:14, padding:"12px 16px", background:"rgba(255,255,255,0.06)", borderRadius:10 }}>
          <p style={{ color:WHITE, fontSize:13, margin:0, lineHeight:1.7 }}>🔁 <strong style={{ color:YELLOW }}>Same post. Same photo. No edits.</strong> Every new group is more visibility.</p>
        </div>
      </Card>
      <Card>
      ...
      <Card>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14, flexWrap:"wrap", gap:10 }}>
          <h3 style={{ color:NAVY, fontSize:16, fontWeight:800, margin:0 }}>Groups near {activeCity !== "Your City" ? activeCity : "you"}</h3>
        ...
        {(!activeCity || activeCity === "Your City") && (
          <div style={{ marginBottom:14 }}>
            <p style={{ fontSize:13, fontWeight:600, color:NAVY, margin:"0 0 8px" }}>What city do you serve?</p>
            <div style={{ display:"flex", gap:8 }}>
              <input value={cityInput} onChange={e => setCityInput(e.target.value)} placeholder="e.g. East Nashville"
                onKeyDown={e => { if (e.key==="Enter") fetchGroups(cityInput.trim()); }}
                style={{ flex:1, border:"2px solid "+GRAY200, borderRadius:8, padding:"8px 12px", fontSize:14, outline:"none", fontFamily:"inherit" }} />
              <button onClick={() => fetchGroups(cityInput.trim())} style={{ background:NAVY, color:YELLOW, border:"none", borderRadius:8, padding:"8px 16px", fontWeight:700, fontSize:13, cursor:"pointer" }}>Search</button>
            </div>
          </div>
        )}
        ...
        {loading && <p style={{ color:GRAY600, fontSize:14, textAlign:"center", padding:24 }}>🔍 Finding groups near {activeCity}...</p>}
        {!loading && groups.length > 0 && (
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {groups.map((g,i) => <GroupPostRow key={i} group={g} num={i+2} />)}
          </div>
        )}
      </Card>
    </>
    ...
    <div>
      ...
      <Card style={{ minHeight:340, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <div style={slideStyle}>
          ...
          {validationFeedback[q.id] && <div style={{ background:"#FEE2E2", border:"1px solid #FCA5A5", borderRadius:8, padding:"10px 14px", marginBottom:10, fontSize:13, color:"#7F1D1D", lineHeight:1.7, display:"flex", gap:8 }}><span>💬</span><span>{validationFeedback[q.id]}</span></div>}
...
function LeadEngagement({ onBack, city, week1Total, onAmplify }) {
  ...
  function logLead(typeId, count=1) {
    setLog(prev => [...prev, ...Array.from({length:count}, () => ({type:typeId, timestamp:Date.now()}))]);
  ...
  return (
    <div>
      ...
      {!watched && (
        <Card>
          <SectionHeader emoji="🎉" title="Session Complete — Now We Execute" subtitle="Before working your leads, watch this quick training video. It covers exactly what to do with every like, comment, share, and DM." />
          <div style={{ background:"#EFF6FF", borderRadius:12, padding:20, marginBottom:16 }}>
            ...
            <a href="https://tryhousecall-1.wistia.com/medias/indqjc1oov" target="_blank" rel="noopener noreferrer"
              style={{ display:"flex", alignItems:"center", gap:16, background:NAVY, borderRadius:12, padding:"18px 20px", textDecoration:"none", marginBottom:16 }}>
              <div style={{ background:YELLOW, borderRadius:"50%", width:48, height:48, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill={NAVY}><polygon points="5 3 19 12 5 21 5 3"/></svg>
              </div>
              <div>
                <div style={{ color:WHITE, fontWeight:800, fontSize:15, marginBottom:2 }}>Watch Training Video</div>
                <div style={{ color:GRAY400, fontSize:12 }}>Opens in a new tab · ~5 min</div>
              </div>
              <div style={{ marginLeft:"auto" }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={GRAY400} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
              </div>
            </a>
            <p style={{ margin:"0 0 16px", fontSize:13, color:GRAY600, lineHeight:1.7 }}>This video covers the Lead Engagement Playbook your coach walked you through live. Watch it once, then come back and tap the button below.</p>
          ...
          <div style={{ borderTop:"1px solid "+GRAY200, paddingTop:14 }}>
            <p style={{ fontSize:12, color:GRAY400, margin:0 }}>Already watched it before?{" "}
              <button onClick={() => setWatched(true)} style={{ background:"none", border:"none", color:NAVY, fontSize:12, fontWeight:700, cursor:"pointer", textDecoration:"underline", padding:0 }}>Skip to the playbook →</button>
            </p>
          </div>
        </Card>
      )}
          ...
          <div>
              ...
              <Card style={{ background:NAVY, marginBottom:16 }}>
                ...
                {showJobEntry && (
                  <div style={{ marginTop:14, padding:"14px 16px", background:"rgba(255,255,255,0.06)", borderRadius:10, display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
                    ...
                    <Btn onClick={() => { const n=parseInt(jobsInput); if(n>0) addJobs(n); }} disabled={!jobsInput||parseInt(jobsInput)<1} style={{ fontSize:13 }}>Add {jobsInput||""} Job{parseInt(jobsInput)!==1?"s":""}</Btn>
                  </div>
                )}
                <div style={{ marginTop:14, padding:"12px 16px", background:"rgba(255,255,255,0.06)", borderRadius:10 }}>
                ...
                <Card>
                  ...
                  <div style={{ borderTop:"1px solid "+GRAY200, paddingTop:16, marginTop:16 }}>
                    {activeLead.id==="like"
                      ...
                      : <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}><Btn variant="success" onClick={() => { logLead(activeLead.id); reset(); }}>✓ Done — Mark as Worked</Btn><Btn variant="ghost" onClick={reset}>← Try Another</Btn></div>}
                  </div>
                </Card>
              )}
              ...
              {!active && (
                <Card style={{ background:"linear-gradient(135deg, #002942, #003a5c)", textAlign:"center" }}>
                  ...
                  <p style={{ color:GRAY400, fontSize:13, lineHeight:1.7, margin:"0 0 20px" }}>Keep your momentum going. Post in more groups to get more engagement — and more leads to work.</p>
          ...
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:24 }}>
              <div onClick={() => setAppPhase("writechoice")} style={{ background:WHITE, borderRadius:16, boxShadow:"0 2px 12px rgba(0,41,66,0.06)", overflow:"hidden", cursor:"pointer", display:"flex", position:"relative" }}>
                <div style={{ position:"absolute", top:0, left:0, bottom:0, width:Math.max(3,(answeredCount/ALL_QUESTIONS.length)*100)+"%", background:"linear-gradient(to right,rgba(16,185,129,0.12),rgba(16,185,129,0.04))", borderRight:answeredCount>0?"2px solid rgba(16,185,129,0.25)":"none", transition:"width 0.5s cubic-bezier(0.4,0,0.2,1)", pointerEvents:"none" }} />
        ...
        {appPhase==="replicate" && (
          <>
            ...
            <Card>
              ...
              {!groupsLoading && groups20.length > 0 && (
                <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
                  {groups20.map((g,i) => <GroupPostRow key={i} group={g} num={i+2} />)}
                </div>
              )}
            </Card>
            <Card>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:16 }}>
                {[1,2,3,4,5,6,7,8,9].map(n => (
                  <button key={n} onClick={() => setFlag("postCount", n)} style={{ width:48, height:48, borderRadius:10, border:"2px solid "+(postCount===n?NAVY:GRAY200), background:postCount===n?NAVY:WHITE, color:postCount===n?YELLOW:GRAY600, fontWeight:800, fontSize:16, cursor:"pointer", transition:"all 0.15s" }}>{n}</button>
                ))}
              </div>
